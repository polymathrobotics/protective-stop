...
class ProtectiveStopNode(LifecycleNode):
    def __init__(self, node_name):
        ...
        self.declare_parameter('machine_uuid', '', ParameterDescriptor(description='Machine uuid of robot'))
        self.declare_parameter(
            'heartbeat_timeout',
            1.0,
            ParameterDescriptor(description='timeout float value in seconds before pstop is triggered'),
        )
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        self.deactivation_timeout = self.get_parameter('deactivation_timeout').get_parameter_value().double_value

        self.machine_uuid = self.get_parameter('machine_uuid').get_parameter_value().string_value

        self.max_pstop_count = self.get_parameter('max_pstop_count').get_parameter_value().integer_value

        self.is_user_monitored = self.get_parameter('is_user_monitored').get_parameter_value().bool_value
...
Node configured successfully with:
    ...
    def on_activate(self, state: State) -> TransitionCallbackReturn:
        ...
        self.subscriber_pstop = self.create_subscription(ProtectiveStop, PROTECTIVE_STOP_TOPIC, self.pstop_callback, 1)
    ...
    def on_deactivate(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info(colored(f"Deactivating node '{self.get_name()}' in state '{state.label}'.", 'cyan'))
    ...
    def parameters_callback(self, params):
        ...
        return SetParametersResult(successful=True, reason='Parameter update processed successfully.')
    ...
    def set_is_user_monitored(self, is_user_monitored: bool):
        ...
        warning_str = (
            'User Monitor Mode DISABLED. If you lose connection to your protective stop on machine, it will continue to produce a heartbeat.'
            if not self.is_user_monitored
            else 'User Monitor Mode ENABLED.'
        )
    ...
    def state_transition_callback(self, activate: bool, request, response):
        ...
        if activate:
            ...
            pstop_state = PStopModel(
                connection_status=ConnectionStatus(status=PStopRemoteStatusEnum.ACTIVE, message='Activated by remote'),
    ...
    def pstop_callback(self, msg):
        ...
        if uuid == self.machine_uuid:
            ...
            if self.connected_remote_pstop_state[uuid].connection_status.status == PStopRemoteStatusEnum.DEACTIVATED:
                self.get_logger().warn(
            ...
            self.connected_remote_pstop_state[uuid].receive_timestamp = Ros2Time.from_ros_time(self.get_clock().now())
            self.connected_remote_pstop_state[uuid].remote_timestamp = Ros2Time.from_time_msg(msg.stamp)
            self.connected_remote_pstop_state[uuid].counter = self.connected_remote_pstop_state[uuid].counter + 1

            reply_msg = ProtectiveStop()
            reply_msg.version = 0
            reply_msg.pstop_pressed = self.connected_remote_pstop_state[uuid].pstop_pressed
            reply_msg.connection_status = self.connected_remote_pstop_state[uuid].connection_status.to_ros_message()
            ...
            self.get_logger().warn(f'Received message from unknown source: {uuid}', throttle_duration_sec=1.0)
    ...
    def process_heartbeat(self):
        ...
        for uuid, pstop_remote in self._safe_remote_iterator():
            ...
            if self._has_exceeded_deactivation_timeout(pstop_remote.receive_timestamp):
                ...
                continue
            elif self.connected_remote_pstop_state[uuid].connection_status.status == PStopRemoteStatusEnum.UNSTABLE:
                self.get_logger().info(
                    colored(
