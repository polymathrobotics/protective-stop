...
static void wifi_try_next(ml_app_t *app) {
    ...
    ESP_LOGI(TAG, "WiFi trying #%d/%d: %s",
             app->current_wifi_idx + 1, app->wifi_list_count, e->ssid);
...
static void ap_fallback_start(ml_app_t *app) {
    ...
    snprintf(app->ap_ssid, sizeof(app->ap_ssid), "%s-%02x%02x%02x",
             CONFIG_ML_AP_SSID_PREFIX, mac[3], mac[4], mac[5]);

    const char *password = CONFIG_ML_AP_PASSWORD;
    bool open = (strlen(password) < 8);

    ESP_LOGW(TAG, "AP fallback: no STA connection after %d s — hosting %s (%s)",
             CONFIG_ML_AP_FALLBACK_TIMEOUT_S, app->ap_ssid,
             open ? "open" : "WPA2");
    ...
    if (err != ESP_OK) { ESP_LOGE(TAG, "set_mode(APSTA) failed: %s", esp_err_to_name(err)); return; }
    err = esp_wifi_set_config(WIFI_IF_AP, &ap_cfg);
    if (err != ESP_OK) { ESP_LOGE(TAG, "set_config(AP) failed: %s", esp_err_to_name(err)); return; }
...
#else
bool ml_app_in_ap_fallback(const ml_app_t *app) { (void)app; return false; }
const char *ml_app_ap_ssid(const ml_app_t *app) { (void)app; return NULL; }
#endif
...
static bool fleet_ota_checkin(ml_app_t *app, char *out_url, int url_size,
                               char *out_sha256, int *out_size, bool *out_reached) {
    ...
    cJSON_AddStringToObject(body, "state",
        app->tailscale_connected ? "CONNECTED" :
        (app->ml ? "CONNECTING" : "IDLE"));
    ...
    if (au && cJSON_IsBool(au)) {
        ...
        if (backend_val != app->auto_update) {
            ...
            ESP_LOGI(TAG, "OTA: auto_update synced from backend: %s",
                     backend_val ? "ON" : "OFF");
        }
    }
    ...
    if (fw_url && cJSON_IsString(fw_url))
        strncpy(out_url, fw_url->valuestring, url_size - 1);
    if (fw_sha && cJSON_IsString(fw_sha))
        strncpy(out_sha256, fw_sha->valuestring, 64);
    if (fw_size && cJSON_IsNumber(fw_size))
        *out_size = (int)fw_size->valuedouble;

    ESP_LOGI(TAG, "OTA: update available — version %s (%d bytes)",
...
static bool fleet_ota_download_and_apply(ml_app_t *app, const char *fw_url,
                                          const char *expected_sha256, int expected_size) {
    ...
    ESP_LOGI(TAG, "OTA: target partition '%s' (%lu KB)",
             update->label, (unsigned long)update->size / 1024);
...
static esp_err_t handler_fleet_ota_toggle(httpd_req_t *req) {
    ...
    if (backend[0] && api_key[0]) {
        ...
        if (err == ESP_OK && status == 200) {
            ...
            ESP_LOGW(TAG, "Fleet OTA: failed to sync auto_update to backend (err=%s status=%d)",
...
static int load_interval_from_nvs(int default_val) {
    ...
    if (val < ML_OTA_CHECK_INTERVAL_MIN_S) val = default_val;
    /* Enforce the 5-minute ceiling even for a previously-persisted longer
     * interval (units in the field were saved with the old 1800s default). */
    if (val > ML_OTA_CHECK_INTERVAL_MAX_S) val = ML_OTA_CHECK_INTERVAL_MAX_S;
    return (int)val;
}
    ...
    if (len > 0) {
        ...
        if (json) {
            ...
            if (val && cJSON_IsNumber(val)) {
                ...
                if (s > ML_OTA_CHECK_INTERVAL_MAX_S) s = ML_OTA_CHECK_INTERVAL_MAX_S;
                if (s >= ML_OTA_CHECK_INTERVAL_MIN_S) {
...
static esp_err_t handler_verbose_toggle(httpd_req_t *req) {
    ...
    char resp[64];
    snprintf(resp, sizeof(resp), "{\"ok\":true,\"verbose\":%s}",
             app->verbose_log ? "true" : "false");
    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(req, resp);
    return ESP_OK;
}
...
static void fleet_ota_task(void *arg) {
    ...
    ESP_LOGI(TAG, "Fleet OTA task started (interval: %ds, backend: %s)",
             app->check_interval_s, CONFIG_ML_OTA_BACKEND_URL);
...
ml_app_t *ml_app_start(const ml_app_config_t *cfg) {
    ...
    } else if (ml_config_get_nvs_wifi(app->wifi_ssid, sizeof(app->wifi_ssid),
     ...
     */
    if (app->wifi_list_count <= 1
        && strlen(CONFIG_ML_WIFI_SSID) > 0
        && strlen(CONFIG_ML_WIFI_SSID_2) > 0) {
        ...
        if (seed) {
            ...
            if (nvs_open("ml_config", NVS_READWRITE, &h) == ESP_OK) {
                ...
                if (nvs_set_blob(h, "wifi_list", seed, save_len) == ESP_OK) {
                    nvs_commit(h);
                    ESP_LOGI(TAG, "WiFi: seeded NVS list with 2 networks from Kconfig (%s + %s)",
                             seed->entries[0].ssid, seed->entries[1].ssid);
                    app->wifi_list = *seed;
    ...
    if (cfg->try_alt_network) {
        ...
        if (alt == ESP_OK) {
            ...
            ESP_LOGI(TAG, "Alternative network not available (%s) — falling back to WiFi",
                     esp_err_to_name(alt));
        }
    }
    ...
    if (!used_alt_network) {
        wifi_init(app);
        xEventGroupWaitBits(app->wifi_events, WIFI_CONNECTED_BIT,
    ...
    if (strlen(CONFIG_ML_OTA_BACKEND_URL) > 0 && strlen(CONFIG_ML_OTA_API_KEY) > 0) {
        ...
        const httpd_uri_t fleet_uris[] = {
            ...
            { .uri = "/admin/api/fleet-ota/interval",  .method = HTTP_POST, .handler = handler_fleet_ota_interval, .user_ctx = app },
        };
        for (int i = 0; i < 4; i++) {
...
esp_err_t ml_app_add_page(ml_app_t *app, const char *uri,
