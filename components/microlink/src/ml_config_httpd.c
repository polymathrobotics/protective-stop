...
static void config_load_settings(ml_config_ctx_t * ctx)
{
  ...
  if (err == ESP_OK && ctx->settings.version >= 1) {
    ...
    if (ctx->settings.version < ML_CONFIG_SETTINGS_VERSION) {
      ESP_LOGI(TAG, "Migrating settings v%d -> v%d", ctx->settings.version, ML_CONFIG_SETTINGS_VERSION);
      ctx->settings.version = ML_CONFIG_SETTINGS_VERSION;
      /* Save migrated blob so next boot loads full size */
      nvs_set_blob(ctx->nvs, NVS_KEY_SETTINGS, &ctx->settings, sizeof(ml_config_settings_t));
      nvs_commit(ctx->nvs);
    }
  } else {
  ...
  ESP_LOGI(
    TAG,
    "Settings v%d (wifi=%s, prefix=%s, name=%s, max_peers=%d, ctrl=%s, pip=%s)",
...
static void config_load_peers(ml_config_ctx_t * ctx)
{
  ...
  if (err == ESP_OK && stored_len > 0) {
    ...
    bool is_old_format = (stored_len >= 1) && ((stored_len - 1) % sizeof(ml_config_peer_entry_t) == 0) &&
    ...
    if (is_old_format) {
      ...
      if (tmp) {
        ...
        if (err == ESP_OK) {
          ...
          if (count > 0) {
            memcpy(ctx->peer_list.entries, tmp + 1, count * sizeof(ml_config_peer_entry_t));
          }
    ...
    ESP_LOGI(
      TAG, "Peer allowlist loaded: %d peers (filter %s)", ctx->peer_list.count, ctx->filter_enabled ? "ON" : "OFF");
  } else {
    memset(&ctx->peer_list, 0, sizeof(ctx->peer_list));
...
static void config_save_peers(ml_config_ctx_t * ctx)
{
  ...
  size_t save_len = sizeof(ctx->peer_list.count) + ctx->peer_list.count * sizeof(ml_config_peer_entry_t);
...
static void config_save_wifi_list(ml_config_ctx_t * ctx)
{
  ...
  if (err == ESP_OK) {
    nvs_commit(ctx->nvs);
    ESP_LOGI(TAG, "WiFi list saved (%d entries, %d bytes)", ctx->wifi_list.count, (int)save_len);
  } else {
    ESP_LOGE(TAG, "Failed to save WiFi list (%d bytes): %s", (int)save_len, esp_err_to_name(err));
  }
}
...
static esp_err_t handler_post_settings(httpd_req_t * req)
{
  ...
  if (derp_region_changed && ctx->ml != NULL) {
     ...
     * Without this, a locked chip is unreachable from remote peers until reboot
     * (PS-wedge: PS locked region 2, machn's netmap kept PS@9, no bond until PS
     * rebooted). Post the endpoint-update re-advertise now (Stream=false
...
static esp_err_t handler_post_allowed(httpd_req_t * req)
{
  ...
  if (xSemaphoreTake(ctx->peer_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
    ...
    if (fleet_ip != 0u) {
      bool present = false;
      for (int i = 0; i < ctx->peer_list.count; i++) {
        if (ctx->peer_list.entries[i].vpn_ip == fleet_ip) {
          present = true;
          break;
        }
      }
      if (!present && ctx->peer_list.count < ML_CONFIG_MAX_ALLOWED_PEERS) {
        ml_config_peer_entry_t * e = &ctx->peer_list.entries[ctx->peer_list.count];
        e->vpn_ip = fleet_ip;
        memset(e->label, 0, sizeof(e->label));
...
static esp_err_t handler_monitor(httpd_req_t * req)
{
  ...
  #if SOC_TEMP_SENSOR_SUPPORTED
  float temp_c = 0;
  if (ctx->temp_sensor && temperature_sensor_get_celsius(ctx->temp_sensor, &temp_c) == ESP_OK) {
  ...
  if (ml) {
    ...
    for (int i = 0; i < (int)(sizeof(tlist) / sizeof(tlist[0])); i++) {
      if (tlist[i].handle) {
        ...
        cJSON_AddNumberToObject(t, "stack_free", (double)uxTaskGetStackHighWaterMark(tlist[i].handle));
      }
    }
...
static esp_err_t handler_ota(httpd_req_t * req)
{
  ...
  ESP_LOGI(
    TAG,
    "OTA: current partition: '%s' at 0x%lx",
    running ? running->label : "?",
    running ? (unsigned long)running->address : 0);
  ESP_LOGI(
    TAG,
    "OTA: target partition:  '%s' at 0x%lx (size: %lu KB)",
    update->label,
    (unsigned long)update->address,
    (unsigned long)update->size / 1024);
  ESP_LOGI(TAG, "OTA: incoming firmware: %d bytes (%d KB)", req->content_len, req->content_len / 1024);
  ...
  while (remaining > 0) {
    ...
    if (recv_len <= 0) {
      if (recv_len == HTTPD_SOCK_ERR_TIMEOUT) {
        ESP_LOGW(TAG, "OTA: recv timeout at %d%%, retrying...", total_written * 100 / req->content_len);
        continue;
      }
      ESP_LOGE(
        TAG,
        "OTA: connection lost at %d / %d bytes (%d%%)",
        total_written,
        req->content_len,
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "OTA: flash write FAILED at %d bytes: %s", total_written, esp_err_to_name(err));
      free(buf);
      esp_ota_abort(ota_handle);
      s_ota_in_progress = false;
  ...
  if (err != ESP_OK) {
    ...
    s_ota_in_progress = false;
    httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Image validation failed (corrupt or wrong chip?)");
    return ESP_FAIL;
  }
...
static esp_err_t handler_ota_status(httpd_req_t * req)
{
  ...
  if (running && esp_ota_get_state_partition(running, &ota_state) == ESP_OK) {
    const char * state_names[] = {"NEW", "PENDING_VERIFY", "VALID", "INVALID", "ABORTED", "UNDEFINED"};
  ...
  if (invalid) {
    ...
    if (esp_ota_get_state_partition(invalid, &inv_state) == ESP_OK) {
      const char * reasons[] = {"NEW", "PENDING_VERIFY", "VALID", "INVALID", "ABORTED", "UNDEFINED"};
...
static esp_err_t handler_get_wifi(httpd_req_t * req)
{
  ...
  for (int i = 0; i < ctx->wifi_list.count && i < ML_CONFIG_MAX_WIFI_ENTRIES; i++) {
    ...
    cJSON_AddStringToObject(entry, "pass", ctx->wifi_list.entries[i].pass[0] ? "********" : "");
    cJSON_AddItemToArray(arr, entry);
  }
  cJSON_AddNumberToObject(json, "active_idx", ctx->wifi_list.active_idx == 0xFF ? -1 : ctx->wifi_list.active_idx);
...
esp_err_t ml_config_httpd_register_routes(
  ml_config_ctx_t * ctx, microlink_t * ml, httpd_handle_t httpd, const char * prefix)
{
  ...
  ESP_LOGI(
    TAG, "Admin routes registered with prefix '%s' (%d handlers)", pfx, (int)(sizeof(routes) / sizeof(routes[0])));
  return ESP_OK;
}
