...
static void wifi_debounce_cb(TimerHandle_t timer)
{
  ...
  ESP_LOGW(TAG, "WiFi down for %dms — triggering switch to cellular", WIFI_DISCONNECT_DEBOUNCE_MS);
  ...
  if (event_base == WIFI_EVENT) {
    if (event_id == WIFI_EVENT_STA_START) {
      ...
      if (s_ctx.wifi_retry_count < 5) {
        ...
        if (s_ctx.wifi_retry_count == 1 && s_ctx.state == ML_NET_SW_WIFI_VPN_UP && s_ctx.wifi_debounce_timer) {
...
static esp_err_t wifi_start(void)
{
  ...
  EventBits_t bits = xEventGroupWaitBits(
    s_ctx.wifi_events, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT, pdFALSE, pdFALSE, pdMS_TO_TICKS(s_ctx.wifi_timeout_ms));
...
static esp_err_t cellular_start(void)
{
  ...
  if (ml_config_get_nvs_ppp(nvs_ppp_user, sizeof(nvs_ppp_user), nvs_ppp_pass, sizeof(nvs_ppp_pass))) {
  ...
  ESP_LOGI(TAG, "Cellular connected via %s", mode == ML_DATA_MODE_PPP ? "PPP" : "AT socket bridge");
  return ESP_OK;
}
...
static esp_err_t vpn_start(void)
{
  ...
  for (int i = 0; i < timeout_ticks; i++) {
    if (microlink_is_connected(s_ctx.ml)) {
      ...
      ESP_LOGI(
        TAG, "VPN connected via %s (IP: %s)", s_ctx.active_transport == ML_NET_WIFI ? "WiFi" : "Cellular", ip_str);
      return ESP_OK;
    }
      ...
      case ML_NET_SW_SWITCHING_TO_CELL: {
        ...
        if (cell_err == ESP_OK) {
          ...
          if (rb_err == ESP_OK) {
            ...
            ESP_LOGE(TAG, "Rebind failed (%s), full restart", esp_err_to_name(rb_err));
            vpn_stop();
            wifi_stop();
            goto try_cellular;
          }
      ...
      case ML_NET_SW_SWITCHING_TO_WIFI: {
        ...
        if (wifi_err == ESP_OK) {
          ...
          if (rb_err == ESP_OK) {
            ...
            ESP_LOGW(TAG, "Rebind to WiFi failed (%s), reverting to cellular", esp_err_to_name(rb_err));
            wifi_stop();
            vpn_stop();
            goto try_cellular;
          }
...
esp_err_t ml_net_switch_init(const ml_net_switch_config_t * config)
{
  ...
  s_ctx.health_timer =
    xTimerCreate("net_health", pdMS_TO_TICKS(s_ctx.health_check_interval_ms), pdTRUE, NULL, health_timer_cb);
  if (!s_ctx.health_timer) return ESP_ERR_NO_MEM;

  /* Create failback timer */
  if (s_ctx.failback_check_ms > 0) {
    s_ctx.failback_timer =
      xTimerCreate("net_failback", pdMS_TO_TICKS(s_ctx.failback_check_ms), pdTRUE, NULL, failback_timer_cb);
  }

  /* Create WiFi disconnect debounce timer (one-shot) */
  s_ctx.wifi_debounce_timer =
    xTimerCreate("wifi_debounce", pdMS_TO_TICKS(WIFI_DISCONNECT_DEBOUNCE_MS), pdFALSE, NULL, wifi_debounce_cb);
  ...
  ESP_LOGI(
    TAG,
    "Net switch initialized (wifi_timeout=%lums, health=%lums, failback=%lums)",
...
esp_err_t ml_net_switch_start(void)
{
  ...
  BaseType_t ret = xTaskCreate(net_switch_task, "net_sw", NET_SW_STACK_SIZE, NULL, 5, &s_ctx.task_handle);
  if (ret != pdPASS) {
    s_ctx.running = false;
...
const char * ml_net_switch_state_str(ml_net_switch_state_t state)
{
  switch (state) {
    case ML_NET_SW_IDLE:
      return "IDLE";
    case ML_NET_SW_WIFI_CONNECTING:
      return "WIFI_CONNECTING";
    case ML_NET_SW_WIFI_VPN_UP:
      return "WIFI_VPN_UP";
    case ML_NET_SW_SWITCHING_TO_CELL:
      return "SWITCHING_TO_CELL";
    case ML_NET_SW_CELL_CONNECTING:
      return "CELL_CONNECTING";
    case ML_NET_SW_CELL_VPN_UP:
      return "CELL_VPN_UP";
    case ML_NET_SW_SWITCHING_TO_WIFI:
      return "SWITCHING_TO_WIFI";
    case ML_NET_SW_ERROR:
      return "ERROR";
    default:
      return "UNKNOWN";
  }
