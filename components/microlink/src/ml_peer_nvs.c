...
void ml_peer_nvs_deinit(void) {
    ...
    if (s_dirty) { (void)flush_table(); s_dirty = false; }
    nvs_close(s_nvs);
    s_initialized = false;
    if (s_table) {
        free(s_table);
...
esp_err_t ml_peer_nvs_save(const ml_peer_t *peer) {
    ...
    if (slot >= 0) {
        ...
        uint16_t min_lru = 0;
        for (int i = 0; i < s_table->count; i++) {
            if (s_protected_vpn_ip != 0 &&
                s_table->entries[i].vpn_ip == s_protected_vpn_ip) {
                continue;
            }
        ...
        if (lru_idx < 0) { lru_idx = 0; }  /* all protected (degenerate) */
    ...
    s_dirty = true;
    ESP_LOGD(TAG, "Saved peer %s (slot=%d/%d, flush deferred)",
...
esp_err_t ml_peer_nvs_flush_if_due(uint64_t now_ms) {
    ...
    if ((now_ms - s_last_flush_ms) < PEER_NVS_FLUSH_INTERVAL_MS) { return ESP_OK; }
