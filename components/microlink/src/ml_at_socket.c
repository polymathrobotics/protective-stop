...
static void * bin_search(const void * haystack, size_t haystack_len, const char * needle, size_t needle_len)
{
...
static int at_cmd(const char * cmd, char * resp, size_t resp_size, int timeout_ms)
{
  ...
  while ((esp_timer_get_time() - start) < timeout_us) {
    ...
    int len = uart_read_bytes(UART_NUM, (uint8_t *)(resp + total), resp_size - total - 1, pdMS_TO_TICKS(20));
    if (len > 0) {
      total += len;
      resp[total] = '\0';
      /* Binary-safe: scan full buffer including past null bytes */
      if (
        bin_search(resp, total, "\r\nOK\r\n", 6) || bin_search(resp, total, "\nOK\r\n", 5) ||
        bin_search(resp, total, "\r\nERROR\r\n", 9) || bin_search(resp, total, "\r\n+CME ERROR", 12) ||
        bin_search(resp, total, "\r\n+CMS ERROR", 12))
      {
        break;
      }
 ...
 * Caller must hold at_mutex. */
static int at_cmd_wait_urc(const char * cmd, const char * urc_prefix, char * resp, size_t resp_size, int timeout_ms)
{
  uart_flush_input(UART_NUM);
  ...
  while ((esp_timer_get_time() - start) < timeout_us) {
    int len = uart_read_bytes(UART_NUM, (uint8_t *)(resp + total), resp_size - total - 1, pdMS_TO_TICKS(100));
    if (len > 0) {
      total += len;
      resp[total] = '\0';
      ...
      if (strstr(resp, urc_prefix)) {
        ...
        len = uart_read_bytes(UART_NUM, (uint8_t *)(resp + total), resp_size - total - 1, pdMS_TO_TICKS(200));
        if (len > 0) {
          total += len;
          resp[total] = '\0';
        }
...
static int at_send_raw(const void * data, size_t len, int timeout_ms)
{
  ...
  while ((esp_timer_get_time() - start) < timeout_us) {
    int n = uart_read_bytes(UART_NUM, (uint8_t *)(resp + total), sizeof(resp) - total - 1, pdMS_TO_TICKS(100));
    if (n > 0) {
      total += n;
      resp[total] = '\0';
...
static int at_read_data_chunk(int link_num, int max_bytes, int * out_remaining)
{
  ...
  while ((esp_timer_get_time() - start) < timeout_us) {
    int n = uart_read_bytes(UART_NUM, (uint8_t *)(hdr + hdr_total), sizeof(hdr) - hdr_total - 1, pdMS_TO_TICKS(20));
    if (n > 0) {
      hdr_total += n;
      hdr[hdr_total] = '\0';
  ...
  ESP_LOGI(
    TAG,
    "CHUNK: link=%d req=%d got=%d remaining=%d %lldms",
    link_num,
    req_len,
    total_written,
    remaining,
...
int ml_at_socket(int domain, int type, int protocol)
{
  ...
  for (int i = 0; i < AT_SOCK_MAX; i++) {
    if (s_at.socks[i].link_num < 0) {
      ...
      ESP_LOGI(TAG, "socket(%s) -> fd=%d link=%d", type == SOCK_STREAM ? "TCP" : "UDP", fd, i);
      return fd;
    }
  }
...
int ml_at_connect(int fd, const struct sockaddr * addr, socklen_t addrlen)
{
  ...
  if (s->type == SOCK_DGRAM && s->local_port > 0) {
    ...
    snprintf(cmd, sizeof(cmd), "AT+CIPOPEN=%d,\"%s\",\"%s\",%u,%u", s->link_num, proto, ip_str, port, s->local_port);
  } else {
    snprintf(cmd, sizeof(cmd), "AT+CIPOPEN=%d,\"%s\",\"%s\",%u", s->link_num, proto, ip_str, port);
  }
...
ssize_t ml_at_send(int fd, const void * buf, size_t len, int flags)
{
  ...
  while (total_sent < len) {
    ...
    while ((esp_timer_get_time() - start) < (int64_t)AT_TIMEOUT_MS * 1000) {
      int n = uart_read_bytes(UART_NUM, (uint8_t *)(resp + resp_len), sizeof(resp) - resp_len - 1, pdMS_TO_TICKS(50));
      if (n > 0) {
        resp_len += n;
        resp[resp_len] = '\0';
        if (strchr(resp, '>')) {
...
ssize_t ml_at_sendto(
  int fd, const void * buf, size_t len, int flags, const struct sockaddr * dest_addr, socklen_t addrlen)
{
  ml_at_sock_t * s = fd_to_sock(fd);
  if (!s) {
    errno = EBADF;
  ...
  if (!s->connected) {
    char cmd[192];
    if (s->local_port > 0) {
      snprintf(cmd, sizeof(cmd), "AT+CIPOPEN=%d,\"UDP\",,,%u", s->link_num, s->local_port);
    } else {
      snprintf(cmd, sizeof(cmd), "AT+CIPOPEN=%d,\"UDP\",,,0", s->link_num);
    }
  ...
  char cmd[192];
  snprintf(cmd, sizeof(cmd), "AT+CIPSEND=%d,%d,\"%s\",%u", s->link_num, (int)chunk, ip_str, port);
...
ssize_t ml_at_recvfrom(int fd, void * buf, size_t len, int flags, struct sockaddr * src_addr, socklen_t * addrlen)
{
  ml_at_sock_t * s = fd_to_sock(fd);
  if (!s) {
    errno = EBADF;
...
int ml_at_setsockopt(int fd, int level, int optname, const void * optval, socklen_t optlen)
{
  ml_at_sock_t * s = fd_to_sock(fd);
  if (!s) {
    errno = EBADF;
...
int ml_at_select(int nfds, fd_set * readfds, fd_set * writefds, fd_set * exceptfds, struct timeval * timeout)
{
...
int ml_at_getaddrinfo(
  const char * hostname, const char * service, const struct addrinfo * hints, struct addrinfo ** res)
{
  ...
  *res = NULL;
  ...
  if (!p) {
    ...
    if (err_p) {
      ...
      ESP_LOGW(
        TAG,
        "DNS failed for %s (no +CDNSGIP response): %.*s",
        hostname,
        (int)strlen(resp) > 200 ? 200 : (int)strlen(resp),
        resp);
    }
    return EAI_NONAME;
  }
