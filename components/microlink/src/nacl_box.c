...
static void cached_beforenm(uint8_t *shared_out,
                            const uint8_t *peer_pk, const uint8_t *our_sk) {
    if (s_shared_cache == NULL) {
        s_shared_cache = heap_caps_calloc(NACL_SHARED_CACHE_SIZE,
                                          sizeof(nacl_shared_ent_t),
    ...
    for (int i = 0; i < NACL_SHARED_CACHE_SIZE; i++) {
        ...
        if (!s_shared_cache[i].valid && free_idx < 0) { free_idx = i; }
        if (s_shared_cache[i].lru < lru_min) { lru_min = s_shared_cache[i].lru; lru_idx = i; }
    }
