...
static void hmac_blake2s(uint8_t out[32], const uint8_t * key, size_t key_len, const uint8_t * data, size_t data_len)
{
...
static void noise_hkdf(
  const uint8_t * ck, const uint8_t * input, size_t input_len, uint8_t * out1, uint8_t * out2, uint8_t * out3)
{
...
static int chacha20poly1305_encrypt(
  const uint8_t * key,
  uint64_t nonce,
  const uint8_t * ad,
  size_t ad_len,
  ...
  uint8_t * ciphertext)
{
  ...
  int ret = mbedtls_chachapoly_encrypt_and_tag(
    &ctx, pt_len, nonce_bytes, ad, ad_len, plaintext, ciphertext, ciphertext + pt_len);

  mbedtls_chachapoly_free(&ctx);
  return ret;
}

static int chacha20poly1305_decrypt(
  const uint8_t * key,
  uint64_t nonce,
  const uint8_t * ad,
  size_t ad_len,
  ...
  uint8_t * plaintext)
{
  ...
  int ret = mbedtls_chachapoly_auth_decrypt(&ctx, payload_len, nonce_bytes, ad, ad_len, tag, ciphertext, plaintext);
  ...
  ml_noise_state_t * state, const uint8_t * local_private, const uint8_t * local_public, const uint8_t * remote_public)
{
  ...
  noise_hash(state->h, (const uint8_t *)NOISE_PROTOCOL_NAME, strlen(NOISE_PROTOCOL_NAME));
  ...
  char prologue[64];
  int prologue_len = snprintf(prologue, sizeof(prologue), "Tailscale Control Protocol v%d", ML_CTRL_PROTOCOL_VER);
...
esp_err_t ml_noise_write_msg1(ml_noise_state_t * state, uint8_t * out, size_t * out_len)
{
  ...
  chacha20poly1305_encrypt(k, 0, state->h, 32, state->local_static_public, 32, out + pos);
  ...
  chacha20poly1305_encrypt(k, 0, state->h, 32, NULL, 0, out + pos);
...
esp_err_t ml_noise_read_msg2(ml_noise_state_t * state, const uint8_t * msg, size_t len)
{
  ...
  if (chacha20poly1305_decrypt(k, 0, state->h, 32, msg + offset, ciphertext_len, NULL) != 0) {
...
esp_err_t ml_noise_encrypt(
  ...
  uint8_t * ciphertext)
{
  if (chacha20poly1305_encrypt(key, nonce, ad, ad_len, plaintext, pt_len, ciphertext) != 0) {
...
esp_err_t ml_noise_decrypt(
  ...
  uint8_t * plaintext)
{
  if (chacha20poly1305_decrypt(key, nonce, ad, ad_len, ciphertext, ct_len, plaintext) != 0) {
