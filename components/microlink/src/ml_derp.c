        ...
        /* ---- Handle DERP connect request from coord task ---- */
        {
            ...
            if ((bits & ML_EVT_DERP_CONNECT_REQ) && !ml->derp.connected) {
                ...
                for (int attempt = 0; attempt < 3 && !ml->derp.connected; attempt++) {
                    if (attempt > 0) {
                        ESP_LOGW(TAG, "DERP connect retry %d/3 in %lums...",
             ...
             * settled and a retry succeeds. One attempt per minute. */
            {
                ...
                if (has_prio) {
                    retry_gap = (s_derp_retry_burst < 2) ? 5000u
                              : (s_derp_retry_burst < 3) ? 10000u : 60000u;
                }
                if (!ml->derp.connected && ml->state == ML_STATE_CONNECTED &&
                    loop_start - s_last_derp_retry_ms > retry_gap) {
                    s_last_derp_retry_ms = loop_start;
                    if (s_derp_retry_burst < 100) { s_derp_retry_burst++; }
                    ESP_LOGI(TAG, "DERP periodic retry (disconnected, burst=%d)",
                             s_derp_retry_burst);
                    if (ml_derp_connect(ml) == ESP_OK) {
                        connected_since_ms = ml_get_time_ms();
                        verbose_phase = true;
...
esp_err_t ml_derp_connect(microlink_t *ml) {
    ...
    ESP_LOGW(TAG, "[TIMING-DERP] DNS: %lld ms (host=%s ip-family=%d)",
             (t_derp_dns - t_derp_start) / 1000, derp_host, res->ai_family);
    ...
    #define DERP_CONNECT_FAIL_CLEANUP() do { \
        if (tls_inited) { \
     ...
     * into the DERP binary frame stream (matching v1 approach) */
    {
        ...
        while (resp_len < (int)sizeof(resp_buf) - 1) {
            ...
            if (now_ms - http_start > DERP_CONNECT_TIMEOUT_MS) {
                ESP_LOGE(TAG, "HTTP upgrade response timeout (resp_len=%d after %llums)",
                         resp_len, now_ms - http_start);
                if (resp_len > 0) {
                    resp_buf[resp_len] = 0;
            ...
            if (now_ms - last_diag_ms > 2000) {
                ESP_LOGW(TAG, "[TIMING-DERP] HTTP-upgrade waiting: %llums, %d bytes so far",
                         now_ms - http_start, resp_len);
                last_diag_ms = now_ms;
            }
