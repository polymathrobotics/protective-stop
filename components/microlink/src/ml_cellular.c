...
static int at_send_cmd(const char * cmd, char * response, size_t resp_size, int timeout_ms)
{
  ...
  while ((esp_timer_get_time() - start) < timeout_us) {
    int len =
      uart_read_bytes(UART_NUM, (uint8_t *)(response + total_read), resp_size - total_read - 1, pdMS_TO_TICKS(100));
    if (len > 0) {
      total_read += len;
      response[total_read] = '\0';

      if (
        strstr(response, "OK") || strstr(response, "ERROR") || strstr(response, "+CME ERROR") ||
        strstr(response, "+CMS ERROR") || strstr(response, "CONNECT"))
      {
        break;
      }
    }
...
static esp_err_t uart_init(void)
{
  ...
  err = uart_set_pin(UART_NUM, s_cell.config.tx_pin, s_cell.config.rx_pin, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
  if (err != ESP_OK) {
  ...
  ESP_LOGI(
    TAG,
    "UART%d: TX=GPIO%d, RX=GPIO%d, Baud=%d",
    UART_NUM,
    s_cell.config.tx_pin,
    s_cell.config.rx_pin,
    s_cell.config.baud_rate);
...
static esp_err_t network_register(void)
{
  ...
  for (int i = 0; i < 30; i++) {
    ...
    if (check_registration(resp)) {
      ...
      ESP_LOGW(
        TAG,
        "EPS registered for SMS only (stat=6) — no data service. "
...
static esp_err_t ppp_setup_and_dial(void)
{
  ...
  {
    const char * apn = (s_cell.config.apn && s_cell.config.apn[0]) ? s_cell.config.apn : "";
    ...
    if (apn[0]) {
      ...
      ESP_LOGW(
        TAG,
        "PDP context: no APN set (using network default) — "
  ...
  const char * ppp_pass = s_cell.config.ppp_pass ? s_cell.config.ppp_pass : "";
  {
    ...
    char auth_cmd[128];
    snprintf(auth_cmd, sizeof(auth_cmd), "AT+CGAUTH=1,%d,\"%s\",\"%s\"", auth_type, ppp_user, ppp_pass);
  ...
  esp_err_t err = esp_event_handler_instance_register(
    IP_EVENT, ESP_EVENT_ANY_ID, ppp_ip_event_handler, NULL, &s_cell.ip_event_handler);
  if (err != ESP_OK) {
  ...
  esp_netif_auth_type_t auth = (ppp_user[0] || ppp_pass[0]) ? NETIF_PPP_AUTHTYPE_CHAP : NETIF_PPP_AUTHTYPE_PAP;
  ...
  esp_event_handler_instance_register(
    NETIF_PPP_STATUS, ESP_EVENT_ANY_ID, ppp_status_event_handler, NULL, &s_cell.ppp_status_handler);
  ...
  BaseType_t ret = xTaskCreatePinnedToCore(ppp_rx_task, "ml_ppp_rx", 4096, NULL, 5, &s_cell.ppp_task, 0);
  if (ret != pdPASS) {
  ...
  if (bits & (PPP_GOT_IP_BIT | PPP_GOT_IP6_BIT)) {
    ESP_LOGI(TAG, "PPP data connection established (%s)", (bits & PPP_GOT_IP_BIT) ? "IPv4" : "IPv6");
    return ESP_OK;
  }
...
esp_err_t ml_cellular_init(const ml_cellular_config_t * config)
{
  ...
  ESP_LOGI(
    TAG,
    "Cellular modem ready: %s IMEI=%s Operator=%s RSSI=%d",
    s_cell.info.model,
    s_cell.info.imei,
...
{
  if (s_cell.state < ML_CELL_STATE_REGISTERED) {
    ESP_LOGE(TAG, "Cannot connect data: not registered on network (state=%d)", s_cell.state);
    return ESP_ERR_INVALID_STATE;
  }
  ...
  ESP_LOGW(TAG, "PPP failed (%s) — falling back to AT socket bridge", esp_err_to_name(ret));
  ml_cellular_ppp_stop();
