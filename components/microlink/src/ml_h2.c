...
static int write_frame_header(uint8_t * out, uint32_t length, uint8_t type, uint8_t flags, uint32_t stream_id)
{
...
static int hpack_literal_indexed(uint8_t * out, int name_index, const char * value)
{
  int pos = 0;
...
int ml_h2_build_preface(uint8_t * out, size_t out_size)
{
  ...
  ESP_LOGI(TAG, "H2 preface built: %d bytes (INITIAL_WINDOW_SIZE=%lu)", pos, (unsigned long)window_size);
  return pos;
}
  ...
  bool end_stream)
{
  ...
  ESP_LOGD(TAG, "H2 HEADERS frame: %d bytes (stream=%lu, hpack=%d)", pos, (unsigned long)stream_id, hpack_len);
  return pos;
}
  ...
  uint8_t * out, size_t out_size, const uint8_t * data, size_t data_len, uint32_t stream_id, bool end_stream)
{
  ...
  ESP_LOGD(TAG, "H2 DATA frame: %d bytes (stream=%lu, end=%d)", pos, (unsigned long)stream_id, end_stream);
  return pos;
}
