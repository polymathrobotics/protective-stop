  ...
  microlink_t * ml, uint32_t dest_ip, uint16_t dest_port, uint32_t timeout_ms)
{
  ...
  ESP_LOGI(TAG, "TCP connect to %s:%u (timeout=%lums)", ip_str, dest_port, (unsigned long)timeout_ms);
  ...
  if (tunnel_up) {
    ...
    ESP_LOGW(TAG, "WG tunnel not up after %lums, attempting connect anyway", (unsigned long)max_wait);
  }
  ...
  ESP_LOGI(TAG, "TCP connected to %s:%u (%lld ms)", ip_str, dest_port, (t_end - t_start) / 1000);
...
esp_err_t microlink_tcp_send(microlink_tcp_socket_t * sock, const void * data, size_t len)
{
  ...
  while (sent < len) {
    ...
    if (n <= 0) {
      ...
      ESP_LOGE(TAG, "TCP send failed: n=%d errno=%d (sent %d/%d)", n, errno, (int)sent, (int)len);
