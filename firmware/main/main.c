...
{
  return estop_channel_closed(core_id, in->counter) ? PSTOP_MESSAGE_OK : PSTOP_MESSAGE_STOP;
}
...
static void ensure_socket_for_peer(void)
{
  ...
  for (;;) {
    ...
    if ((peer_ip & 0xFFC00000u) != 0x64400000u) {
      want = 0u;
      break;
    }
    want = dcs_get_vpn_ip();
    ...
    ESP_LOGW(
      TAG,
      "pstop peer is a Tailscale IP but VPN is not up — "
      "holding bond (machine sees silence = fail-safe STOP)");
    vTaskDelay(pdMS_TO_TICKS(2000));
  }

  if ((g_sock >= 0) && (want == s_sock_local_ip)) {
    return;
  }
  if (g_sock >= 0) {
    close(g_sock);
...
{
  int n = sendto(g_sock, bytes, PSTOP_MESSAGE_SIZE, 0, (struct sockaddr *)&g_peer, sizeof(g_peer));
...
static void drain_socket(void)
{
  for (;;) {
    ...
    if (recvfrom(g_sock, junk, sizeof(junk), 0, NULL, NULL) <= 0) {
      break;
    }
  }
}
...
static void bond_with_retry(protocol_data_t * pd)
{
  ...
  for (;;) {
    ...
    ESP_LOGI(
      TAG, "BOND ok: machine counter=%lu stamp=%llu", (unsigned long)resp.counter, (unsigned long long)resp.stamp);
...
static void comparator_task(void * arg)
{
  ...
  for (;;) {
    ...
    if ((now_ms - last_reply_ms) > REBOND_AFTER_MS) {
      ESP_LOGW(
        TAG, "no machine reply for %llu ms — re-bonding to re-sync", (unsigned long long)(now_ms - last_reply_ms));
