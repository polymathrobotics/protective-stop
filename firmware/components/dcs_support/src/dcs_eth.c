...
static void on_eth_got_ip(void *arg, esp_event_base_t base, int32_t id, void *data) {
    ...
    ESP_LOGI(TAG, "W5500 got IP " IPSTR " gw " IPSTR,
...
esp_err_t dcs_eth_start(void) {
    ...
    ESP_LOGI(TAG, "W5500 Ethernet started (SPI%d cs=%d int=%d rst=%d @%dMHz, "
             "MAC %02x:%02x:%02x:%02x:%02x:%02x, route_prio=%d)",
             (int)DCS_ETH_SPI_HOST + 1, DCS_ETH_PIN_CS, DCS_ETH_PIN_INT, DCS_ETH_PIN_RST,
...
fail_driver:
    ...
    if (phy != NULL) { (void)phy->del(phy); }
    if (mac != NULL) { (void)mac->del(mac); }
fail_spi:
    if (spi_owned) {
...
bool dcs_eth_link_up(void) { return atomic_load(&s_link_up); }
bool dcs_eth_has_ip(void)  { return atomic_load(&s_has_ip); }
bool dcs_eth_is_enabled(void) { return atomic_load(&s_enabled); }
...
esp_err_t dcs_eth_set_enabled(bool on) {
    ...
    if (!on) { atomic_store(&s_link_up, false); atomic_store(&s_has_ip, false); }
