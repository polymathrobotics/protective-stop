 ...
 *
 *     YELLOW = DISCONNECTED  no fresh reply from the machine (never bonded, or
 *                            it went unreachable / the comparator stopped
 *                            sending after a lockstep fault).
...
#define LINK_FRESH_MS      2000u      /* no machine reply within this -> DISCONNECTED (re-bond is 1.5s) */
...
typedef enum { RING_DISCONNECTED, RING_BOND, RING_OK, RING_STOP, RING_MISMATCH } ring_state_t;
...
static void ring_task(void *arg) {
    ...
    for (;;) {
         ...
         * takes priority; otherwise the colour reflects the machine's most
         * recent reply, or yellow when there's no fresh reply. */
        ring_state_t st;
        if      (recent_mismatch)               { st = RING_MISMATCH; }     /* purple */
        else if (!connected)                    { st = RING_DISCONNECTED; } /* yellow */
        ...
        switch (st) {
        case RING_DISCONNECTED: ring_fill(B, B, 0); break;   /* yellow */
        ...
        if ((int)st != last_logged) {
            static const char *N[] = { "DISCONNECTED", "BOND/UNBOND", "OK", "STOP", "MISMATCH" };
            ESP_LOGI(TAG, "ring -> %s (machine_msg=%u replies=%lu mm=%lu)",
                     N[st], lastmsg, (unsigned long)replies, (unsigned long)mm);
            last_logged = (int)st;
        }
