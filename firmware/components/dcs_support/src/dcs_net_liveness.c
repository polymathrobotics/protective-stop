...
#define NET_LIVENESS_TIMEOUT_MS 180000   /* 180 s of silence (once armed) -> abort.
...
void dcs_net_liveness_stats(uint32_t *rtt_ms, uint32_t *rtt_max_ms,
                            uint32_t *replies, uint32_t *losses) {
    if (rtt_ms != NULL)     { *rtt_ms     = atomic_load(&s_gw_rtt_ms); }
    if (rtt_max_ms != NULL) { *rtt_max_ms = atomic_load(&s_gw_rtt_max_ms); }
    if (replies != NULL)    { *replies    = atomic_load(&s_ping_reply_count); }
    if (losses != NULL)     { *losses     = atomic_load(&s_ping_loss_count); }
}
...
static void on_ping_success(esp_ping_handle_t hdl, void *args) {
    ...
    ESP_LOGI("GW_PING",
             ...
             seqno, ttl, ipaddr_ntoa(&target), recv_len, elapsed_ms);
...
static void retarget_ping(uint32_t gw) {
    ...
    if ((esp_ping_new_session(&cfg, &cbs, &s_ping_handle) == ESP_OK) && (s_ping_handle != NULL)) {
        ...
        ESP_LOGI("GW_PING", "session started -> %s every 5s",
                 ipaddr_ntoa(&target));
    } else {
...
static void net_liveness_task(void *arg) {
    (void)arg;
    ESP_LOGI("LIVENESS", "arm-on-both(gateway+pstop), timeout=%dms, "
        ...
        if (silence_ms > (uint64_t)NET_LIVENESS_TIMEOUT_MS) {
            ...
            if (ps_last == 0u) {
                ESP_LOGW("LIVENESS",
                         ...
                         "wedge, NOT aborting", (unsigned long long)silence_ms);
                continue;
            }
            bool pstop_alive = (now_ms >= ps_last) &&
                               ((now_ms - ps_last) < (uint64_t)NET_LIVENESS_TIMEOUT_MS);
            if (pstop_alive) {
                ESP_LOGW("LIVENESS",
                         "gateway silent %llums but pstop reply is fresh — lwIP "
                         "alive, NOT aborting", (unsigned long long)silence_ms);
                continue;
            }
