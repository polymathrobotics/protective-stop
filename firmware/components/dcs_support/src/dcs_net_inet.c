...
#define INET_DOWN_AFTER_MS      20000u  /* NEITHER target replied this long -> DOWN.
...
bool dcs_net_inet_down(void) { return atomic_load(&s_down) != 0; }
...
static void start_session(void) {
    ...
    if ((esp_ping_new_session(&cfg, &cbs, &s_ping) == ESP_OK) && (s_ping != NULL)) {
        ...
        ESP_LOGI(TAG, "probing %s every %dms", INET_TARGETS[s_target_idx],
                 INET_PING_INTERVAL_MS);
    } else {
...
static void inet_probe_task(void *arg) {
    ...
    for (;;) {
        ...
        if (s_ping == NULL) { start_session(); continue; }
        ...
        if (atomic_load(&s_fail_streak) >= INET_SWITCH_AFTER_FAILS) {
            ...
            ESP_LOGW(TAG, "no reply from current target — switching to %s",
                     INET_TARGETS[s_target_idx]);
            start_session();
        }
