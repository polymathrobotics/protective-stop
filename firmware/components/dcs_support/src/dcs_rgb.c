...
#define BLINK_CYCLE_GAP \
  1500 /* shorter dark gap so a sparse octet
...
static void rgb_task(void * arg)
{
  ...
  for (;;) {
    ...
    if ((int)a != last_logged) {
      ESP_LOGI(TAG, "iface=%d octet=%u colour=(%u,%u,%u)", (int)a, octet, r, g, b);
      last_logged = (int)a;
    }
    ...
    if (((a == DCS_IFACE_ETH) || (a == DCS_IFACE_USB) || (a == DCS_IFACE_WIFI)) && dcs_net_inet_down()) {
      for (int i = 0; i < 6; i++) {
