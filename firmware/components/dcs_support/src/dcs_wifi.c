...
void dcs_wifi_status(int *reason, int *connected, int *idx, int *count) {
    if (reason != NULL)    { *reason    = atomic_load(&s_disc_reason); }
    if (connected != NULL) { *connected = atomic_load(&s_connected); }
    if (count != NULL)     { *count     = (s_list != NULL) ? s_list->count : 0; }
    if (idx != NULL) {
...
static void schedule_reconnect(void) {
    ...
    s_backoff_ms = (s_backoff_ms < (WIFI_BACKOFF_MAX_MS / 2u))
                       ? (s_backoff_ms * 2u) : WIFI_BACKOFF_MAX_MS;
}
...
bool dcs_wifi_is_enabled(void) { return atomic_load(&s_enabled); }
    ...
    if (on) {
        if (!s_inited) {
            ...
            if (pre_inited) {
                ...
                if (r != ESP_OK) {
                    ESP_LOGE(TAG, "esp_wifi_init: %s — internal RAM tight (likely the "
                                  "Tailscale bring-up burst); will retry", esp_err_to_name(r));
                    ...
                    if (s_init_retry == NULL) {
                        esp_timer_create_args_t rt = { .callback = init_retry_cb,
                                                       .name = "dcs_wifi_ir" };
            ...
            esp_timer_create_args_t ta = { .callback = reconnect_timer_cb,
                                           .name = "dcs_wifi_rc" };
        ...
        if ((s_list != NULL) && (!ml_config_get_wifi_list(s_list))) { s_list->count = 0; }
        s_idx = 0;
        ...
        ESP_LOGW(TAG, "WiFi ENABLED (power-save OFF) — cycling %d configured network(s)",
                 (s_list != NULL) ? s_list->count : 0);
        return ESP_OK;
    }
