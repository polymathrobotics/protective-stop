...
void dcs_safety_account_boot(void)
{
  ...
  bool crash_reset = ((g_dcs.reset_reason == (uint8_t)ESP_RST_PANIC) && !was_liveness_abort) ||
                     (g_dcs.reset_reason == (uint8_t)ESP_RST_INT_WDT) ||
                     (g_dcs.reset_reason == (uint8_t)ESP_RST_TASK_WDT) || (g_dcs.reset_reason == (uint8_t)ESP_RST_WDT);
  ...
  if (crash_reset) {
    ...
    ESP_LOGW(TAG, "SAFETY: CRASH boot #%u (reset_reason=%u)", g_dcs.boot_count, g_dcs.reset_reason);
  } else {
    ESP_LOGI(TAG, "SAFETY: clean boot (reset_reason=%u, crash_count=%u)", g_dcs.reset_reason, g_dcs.boot_count);
  }
  ...
  if (next && esp_ota_get_state_partition(next, &other_state) == ESP_OK) {
    other_invalid = ((other_state == ESP_OTA_IMG_INVALID) || (other_state == ESP_OTA_IMG_ABORTED));
  }
  ...
  ESP_LOGE(
    TAG,
    "SAFETY: boot count %u > %u — invoking rollback+reboot "
...
static void bc_clear_task(void * arg)
{
  ...
  g_dcs.boot_count = 0;
  ESP_LOGI(
    TAG, "SAFETY: crash counter cleared after %lus healthy uptime", (unsigned long)(DCS_SAFETY_CLEAR_AFTER_MS / 1000));
  ...
  if (g_dcs.derp_only_mode) {
    ESP_LOGW(
      TAG,
      "SAFETY: DERP-only mode survived %lus healthy — "
