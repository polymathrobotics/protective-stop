...
#define MAX_TASKS       64   /* headroom over the live task count (was 40); the overflow guard in the sampler still protects if exceeded */
...
static uint32_t prev_runtime(const TaskStatus_t *prev, UBaseType_t prev_n,
                              TaskHandle_t h, bool *found) {
...
static void top_insert(top_entry_t *top, int *n, const char *name, uint32_t delta) {
    int pos = *n;
    for (int i = 0; i < *n; i++) {
        if (delta > top[i].delta_us) { pos = i; break; }
    }
    if (pos >= DCS_TOP_N) {
        return;
    }
...
static void load_sampler(void *arg) {
    ...
    for (;;) {
        ...
        atomic_store(&g_dcs_heap_min_internal,
        ...
        if (cur_n == 0u) {
            ...
            ESP_LOGW(TAG, "task snapshot overflow (>%d tasks) — load sample skipped",
                     MAX_TASKS);
            continue;
        }
        ...
        if (primed) {
            ...
            if (d_wall == 0u) { d_wall = 1u; }

            for (int c = 0; c < 2; c++) {
                uint32_t d_idle = idle[c] - prev_idle[c];
                uint32_t load = (d_wall > d_idle)
                    ? (uint32_t)(100u - ((uint64_t)d_idle * 100u) / d_wall)
                    : 0u;
            ...
            for (UBaseType_t i = 0; i < cur_n; i++) {
                bool found = false;
                uint32_t before = prev_runtime(prev_snap, prev_n,
                                                cur_snap[i].xHandle, &found);
                if (!found) {
                    continue;
                }
            ...
            for (int bkt = 0; bkt < 3; bkt++) {
                ...
                for (int i = 0; i < top_n[bkt]; i++) {
                    ...
                    (void)strncpy(out->b[bkt].tasks[i].name, top[bkt][i].name,
                                  sizeof(out->b[bkt].tasks[i].name) - 1u);
