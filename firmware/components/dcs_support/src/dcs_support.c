...
TaskHandle_t dcs_task_spawn_psram(TaskFunction_t fn, const char *name,
                                  uint32_t stack, void *arg,
                                  UBaseType_t prio, BaseType_t core) {
    TaskHandle_t h = NULL;
    if ((xTaskCreatePinnedToCoreWithCaps(fn, name, stack, arg, prio, &h, core,
...
dcs_boot_state_t dcs_support_init(void) {
    ...
    if (g_dcs.ts_boot_en && (g_dcs.boot_count == 1u)) {
        ...
        ESP_LOGW(TAG, "SAFETY: ts_boot=1 but boot_count=%u — forcing Tailscale "
                 "paused for this boot (safe mode)", g_dcs.boot_count);
        g_dcs.ts_boot_en = false;
    } else {
        /* boot_count 0: no safety-ladder action */
    }
    ESP_LOGI(TAG, "Boot: usb_enabled=%d ts_boot_en=%d boot_count=%u",
             g_dcs.usb_enabled, g_dcs.ts_boot_en, g_dcs.boot_count);
    ...
    if (dcs_boot_alt_network_won()) {
        if (dcs_eth_has_ip()) {
            ...
            ESP_LOGI(TAG, "wired Ethernet active — skipping WiFi driver init "
...
void dcs_publish_comparator(uint32_t sent, uint32_t mismatch, uint32_t send_fail,
                            uint64_t last_reply_ms, uint32_t rtt_ms) {
...
void dcs_get_initial_pstop_peer(uint32_t *peer_ip, uint16_t *peer_port) {
    if (peer_ip != NULL)   { *peer_ip   = atomic_load(&g_dcs_pstop_peer_ip); }
    if (peer_port != NULL) { *peer_port = (uint16_t)atomic_load(&g_dcs_pstop_peer_port); }
}
