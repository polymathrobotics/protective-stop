...
static int emit_bucket(char *buf, int cap, const dcs_task_bucket_t *b) {
    ...
    for (int i = 0; (i < (int)b->n) && (len < cap); i++) {
        const char *sep = (i == 0) ? "" : ",";
        len += snprintf(buf + len, cap - len, "%s[\"%s\",%lu]",
                        sep, b->tasks[i].name, (unsigned long)b->tasks[i].pct);
    }
    if (len < cap) {
...
static esp_err_t page_state(httpd_req_t *req) {
    ...
    for (int i = 0; i < rn; i++) {
        rp += snprintf(rsth + rp, sizeof(rsth) - (size_t)rp, "%s%u",
                       (i != 0) ? "," : "", rhist[i]);
        if (rp > (int)(sizeof(rsth) - 1U)) { rp = (int)(sizeof(rsth) - 1U); }
    }
    ...
    int n = snprintf(buf, cap,
        ...
        wifi_disc, wifi_conn, wifi_idx, wifi_n,
        ...
        (unsigned int)g_dcs.boot_count, (unsigned int)g_dcs.reset_reason,
        ml_state, (unsigned long)vpn_ip, (unsigned long)public_ip, derp_region,
        ...
        (unsigned long)heap_min_internal, (unsigned long)pbuf_fails,
        (unsigned long)wg_dedup_drops,
        (unsigned long)gw_rtt, (unsigned long)gw_rtt_max,
        (unsigned long)gw_ok, (unsigned long)gw_loss,
        dcs_net_inet_down() ? 1 : 0, (unsigned long)dcs_net_inet_silent_ms(),
        rsth, (int)ota_st, pstop_num);
    ...
    #define CLAMP_N() do { if (n < 0) { n = 0; } else if (n > (cap - 1)) { n = cap - 1; } } while (0)
    CLAMP_N();
    n += emit_bucket(buf + n, cap - n, &snap.b[0]); CLAMP_N();
    n += snprintf(buf + n, cap - n, ",\"oth0\":%lu,\"tk1\":",
                  (unsigned long)snap.b[0].other_pct); CLAMP_N();
    n += emit_bucket(buf + n, cap - n, &snap.b[1]); CLAMP_N();
    n += snprintf(buf + n, cap - n, ",\"oth1\":%lu,\"tks\":",
                  (unsigned long)snap.b[1].other_pct); CLAMP_N();
    n += emit_bucket(buf + n, cap - n, &snap.b[2]); CLAMP_N();
    n += snprintf(buf + n, cap - n, ",\"oths\":%lu}",
                  (unsigned long)snap.b[2].other_pct); CLAMP_N();
    #undef CLAMP_N
...
static esp_err_t api_wg_toggle(httpd_req_t *req) {
    ...
    char buf[48];
    int n = snprintf(buf, sizeof(buf), "{\"ok\":true,\"paused\":%d}",
                     atomic_load(&g_dcs_wg_paused));
    (void)httpd_resp_set_type(req, "application/json");
    return httpd_resp_send(req, buf, n);
}
...
static esp_err_t api_derp_delay(httpd_req_t *req) {
    ...
    char buf[48];
    int n = snprintf(buf, sizeof(buf), "{\"ok\":true,\"ms\":%d}",
                     microlink_get_derp_loop_delay_ms());
    (void)httpd_resp_set_type(req, "application/json");
    return httpd_resp_send(req, buf, n);
}
...
static esp_err_t api_pstop_peer(httpd_req_t *req) {
    ...
    if ((httpd_query_key_value(query, "ip", ipstr, sizeof(ipstr)) != ESP_OK) ||
...
static esp_err_t api_ts_boot_toggle(httpd_req_t *req) {
    ...
    int n = snprintf(buf, sizeof(buf),
        "{\"ok\":true,\"ts_boot_en\":%d,\"message\":\"takes effect on next reboot\"}",
        new_state ? 1 : 0);
    return httpd_resp_send(req, buf, n);
}
...
static esp_err_t api_usb_enable_toggle(httpd_req_t *req) {
    ...
    char buf[96];
    int n = snprintf(buf, sizeof(buf),
        "{\"ok\":true,\"usb_enabled\":%d,\"message\":\"rebooting in 500 ms\"}",
        new_state ? 1 : 0);
...
static esp_err_t iface_toggle_reply(httpd_req_t *req, esp_err_t r, bool now_on) {
    char buf[80];
    int n = snprintf(buf, sizeof(buf), "{\"ok\":%s,\"enabled\":%d,\"err\":\"%s\"}",
                     (r == ESP_OK) ? "true" : "false", now_on ? 1 : 0, esp_err_to_name(r));
    (void)httpd_resp_set_type(req, "application/json");
    if (r != ESP_OK) {
        (void)httpd_resp_set_status(req, "500 Internal Server Error");
    }
...
static esp_err_t api_enter_download(httpd_req_t *req) {
    ...
    char query[24], c[4] = {0};
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) != ESP_OK ||
        httpd_query_key_value(query, "confirm", c, sizeof(c)) != ESP_OK ||
        strcmp(c, "1") != 0) {
    ...
    ESP_LOGW(TAG, "ENTER DOWNLOAD MODE requested — device will wait for a full "
                  "flash after reset (headless recovery)");
    httpd_resp_set_type(req, "application/json");
    (void)httpd_resp_sendstr(req,
        "{\"ok\":true,\"message\":\"Entering download mode. Full-flash to recover.\"}");
