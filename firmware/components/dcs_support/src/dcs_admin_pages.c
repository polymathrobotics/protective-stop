...
static esp_err_t page_state(httpd_req_t * req)
{
  ...
  int n = snprintf(
    ...
    (unsigned long)atomic_load(&g_dcs_eth_rec_r2),
...
#define CLAMP_N()               \
  ...
  CLAMP_N();
  /* Snapshot time follows all reply loads: a reply published while formatting
   * must never appear to be in the future. This can only overestimate age. */
  n += snprintf(
    buf + n,
    cap - n,
...
static esp_err_t api_ring_offset(httpd_req_t * req)
{
    ...
    (httpd_query_key_value(query, "n", val, sizeof(val)) != ESP_OK))
  {
    (void)httpd_resp_set_status(req, "400 Bad Request");
    (void)httpd_resp_set_type(req, "application/json");
    return httpd_resp_sendstr(req, "{\"ok\":false,\"error\":\"missing ?n=0..15\"}");
  }
  int n = (int)strtol(val, NULL, 10);
  if ((n < 0) || (n > 15)) {
    (void)httpd_resp_set_status(req, "400 Bad Request");
    (void)httpd_resp_set_type(req, "application/json");
    return httpd_resp_sendstr(req, "{\"ok\":false,\"error\":\"n must be 0..15\"}");
  }
  esp_err_t r = dcs_nvs_write_ring_offset((uint8_t)n);
  if (r == ESP_OK) {
    dcs_pstop_ring_set_offset((uint8_t)n); /* live, next repaint */
  }
  char buf[80];
  int len = snprintf(
    buf,
 ...
 * colours; auto-expires (DCS_RING_LOCATE_TIMEOUT_MS) so a forgotten locate
 * can't mask STOP/OK indefinitely. */
static esp_err_t api_ring_led1(httpd_req_t * req)
{
  char query[24], val[4];
  if (
    (httpd_req_get_url_query_str(req, query, sizeof(query)) != ESP_OK) ||
  ...
  bool on = (val[0] == '1');
  dcs_pstop_ring_locate(on);
  char buf[112];
  int len = snprintf(
    buf,
    sizeof(buf),
    "{\"ok\":true,\"ring_led1\":%d,\"ring_offset\":%d,"
    "\"timeout_ms\":%lu}",
    on ? 1 : 0,
    (int)dcs_pstop_ring_get_offset(),
