...
bool dcs_boot_alt_network_won(void) { return s_alt_network_won; }
...
esp_err_t dcs_boot_try_tether_or_skip(uint32_t timeout_ms) {
    ...
    if (e == ESP_OK) {
        if (dcs_eth_wait_for_ip(DCS_ETH_BOOT_WAIT_MS)) {
            ESP_LOGI(TAG, "W5500 Ethernet up with DHCP lease — preferring wired, "
                          "skipping WiFi");
            s_alt_network_won = true;
            return ESP_OK;
        }
        ESP_LOGW(TAG, "W5500 no link/lease in %dms — trying next interface "
                      "(eth stays live for runtime promotion)", DCS_ETH_BOOT_WAIT_MS);
    } else {
        ESP_LOGE(TAG, "dcs_eth_start failed: %s — continuing without Ethernet",
                 esp_err_to_name(e));
    }
    ...
    if (g_dcs.usb_enabled) {
        ...
        ESP_LOGI(TAG, "USB tether: %s (used_alt_network=%d)",
                 esp_err_to_name(r), (int)(r == ESP_OK));
        if (r == ESP_OK) {
            s_alt_network_won = true;
...
bool dcs_usb_is_enabled(void) { return atomic_load(&s_usb_enabled); }
