...
def build_pstop_message(
    message_id, state_id, timestamp, sender_id, receiver_id,
    ...
    Args:
        ...
        sender_id (dict): Sender UUID {'uuid': str}
        receiver_id (dict): Receiver UUID {'uuid': str}
...
def setup_client(client, ip, port, shared):
    ...
    def callback(message):
        ...
        receiver_uuid_raw = message.get("receiver_id", {}).get("uuid")
        ...
        if state_id == 13:  # TRANSITION_STATE_ACTIVATING
            ...
            response = build_pstop_message(
                PSTOP_OK, 13, message["stamp"], message["id"], message["receiver_id"],
            ...
            response = build_pstop_message(
                PSTOP_OK, 14, message["stamp"], message["id"], message["receiver_id"],
                message["counter"], message["heartbeat_timeout"],
                message["received_stamp"], message["received_counter"]
            )
            publisher.publish(roslibpy.Message(response))
...
def client_thread(ip, rosbridge_port, shared, connected_clients):
    ...
    while ip in connected_clients:
        ...
        receiver_id = {"uuid": list(receiver_uuid.bytes)}
        ...
        message = build_pstop_message(
            button, state["id"], timestamp, id, receiver_id, counter,
