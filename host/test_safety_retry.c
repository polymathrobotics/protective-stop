...
int main(void)
{
  ...
  check_ladder(machine_a, NULL, 0, true);

  /* Registration survives loss of health and need not be in the first slot. */
  ...
  peers[0].ip = 0;
  check_ladder(0, peers, count, false);
  check_ladder(machine_a, peers, count, true);
  ...
  /* Eligibility changes do not replenish the bounded retry burst. */
  ...
  /* Preserve attempt-based fallback: faster attempts can reach it sooner. */
