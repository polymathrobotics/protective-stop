...
static uint64_t parse_uint(const char *v) { return strtoull(v, NULL, 0); }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (*s == '[') {
            if (s[1] == '[') {                       /* [[operator]] */
                ...
                if (c->n_operators < (int)MAX_OPERATORS) {
                    ...
                    op->device_id = 0; op->heartbeat_ms = 0;  /* 0 => default */
                    op->stop_only = 0; op->allowed = 1;
                } else {
        ...
        *eq = 0;
        ...
        if (strcmp(section, "operator") == 0 && op) {
            if      (!strcmp(key, "device_id"))    op->device_id    = (uint32_t)parse_uint(val);
            else if (!strcmp(key, "heartbeat_ms")) op->heartbeat_ms = parse_uint(val);
            else if (!strcmp(key, "stop_only"))    op->stop_only    = parse_bool(val);
            else if (!strcmp(key, "allowed"))      op->allowed      = parse_bool(val);
            else fprintf(stderr, "  config: unknown operator key '%s'\n", key);
            continue;
        }

        /* Flat keys matched by name (section is documentation only). */
        if      (!strcmp(key, "port"))                  c->port                  = (int)parse_uint(val);
        else if (!strcmp(key, "bind"))                  snprintf(c->bind_addr, sizeof(c->bind_addr), "%s", val);
        else if (!strcmp(key, "machine_device_id"))     c->machine_device_id     = (uint32_t)parse_uint(val);
        else if (!strcmp(key, "max_lost_messages"))     c->max_lost_messages     = (uint16_t)parse_uint(val);
        else if (!strcmp(key, "max_missed_heartbeats")) c->max_missed_heartbeats = (uint16_t)parse_uint(val);
        else if (!strcmp(key, "max_remotes"))           c->max_remotes           = (uint16_t)parse_uint(val);
        else if (!strcmp(key, "verbose"))               c->verbose               = parse_bool(val);
        else if (!strcmp(key, "allow_unlisted"))        c->allow_unlisted        = parse_bool(val);
        else if (!strcmp(key, "default_heartbeat_ms"))  c->default_heartbeat_ms  = parse_uint(val);
        else if (!strcmp(key, "default_stop_only"))     c->default_stop_only     = parse_bool(val);
        else if (!strcmp(key, "min_stop_ms"))           c->min_stop_ms           = parse_uint(val);
        else fprintf(stderr, "  config: unknown key '%s' (section [%s])\n", key, section);
    }
    fclose(f);
    return 0;
}
...
static void cfg_dump(const machine_cfg_t *c) {
    fprintf(stderr,
            ...
            "monotonic clock)\n",
            c->bind_addr, c->port, c->machine_device_id, c->max_lost_messages,
            c->max_missed_heartbeats, c->max_remotes, c->verbose);
    fprintf(stderr, "config: policy allow_unlisted=%d default_heartbeat=%llums "
            "default_stop_only=%d; %d explicit operator(s):\n",
            c->allow_unlisted, (unsigned long long)c->default_heartbeat_ms,
            c->default_stop_only, c->n_operators);
    for (int i = 0; i < c->n_operators; i++)
        fprintf(stderr, "config:   operator 0x%08X heartbeat=%llums stop_only=%d allowed=%d\n",
                c->operators[i].device_id,
                (unsigned long long)(c->operators[i].heartbeat_ms ?
                    c->operators[i].heartbeat_ms : c->default_heartbeat_ms),
                c->operators[i].stop_only, c->operators[i].allowed);
}
...
static void on_sig(int signo) { (void)signo; s_running = 0; }
...
static remote_details_t is_operator_allowed(const device_id_t *device_id) {
    ...
    for (int i = 0; i < g_cfg.n_operators; i++) {
        if (g_cfg.operators[i].device_id == device_id->data) {
            ...
            d.heartbeat_ms = g_cfg.operators[i].heartbeat_ms ?
                             g_cfg.operators[i].heartbeat_ms : g_cfg.default_heartbeat_ms;
            return d;
        }
    }
...
static const char *msg_name(uint8_t m) {
    switch (m) {
    case PSTOP_MESSAGE_OK:     return "OK";
    case PSTOP_MESSAGE_STOP:   return "STOP";
    case PSTOP_MESSAGE_BOND:   return "BOND";
    case PSTOP_MESSAGE_UNBOND: return "UNBOND";
    default:                   return "UNKNOWN";
    }
}
static const char *rstate_name(int s)  { return s == ROBOT_STATE_OK ? "OK" : "STOPPED"; }
static const char *restart_name(int s) {
    switch (s) {
    case ROBOT_RESTART_STATE_OK:            return "OK";
    case ROBOT_RESTART_STATE_NEED_STOP:     return "NEED_STOP";
    case ROBOT_RESTART_STATE_STOP_RECEIVED: return "STOP_RECEIVED";
    default:                                return "?";
    }
}
static const char *err_name(pstop_error_t e) {
    switch (e) {
    case PSTOP_OK:                    return "OK";
    case PSTOP_OPERATOR_NOT_ALLOWED:  return "OPERATOR_NOT_ALLOWED";
    case PSTOP_OUT_OF_OPERATOR_SPACE: return "OUT_OF_OPERATOR_SPACE";
    case PSTOP_MSG_LOST:              return "MSG_LOST";
    case PSTOP_MSG_REPETITION:        return "MSG_REPETITION";
    case PSTOP_MSG_DELAYED:           return "MSG_DELAYED";
    case PSTOP_MSG_OUT_OF_ORDER:      return "MSG_OUT_OF_ORDER";
    case PSTOP_MSG_INVALID_CHECKSUM:  return "INVALID_CHECKSUM";
    case PSTOP_ERROR_INVALID_ID:      return "INVALID_ID";
    case PSTOP_NOT_BOND_MESSAGE:      return "NOT_BOND_MESSAGE";
    case PSTOP_INVALID_BOND_REQUEST:  return "INVALID_BOND_REQUEST";
    case PSTOP_HEARTBEAT_INVALID:     return "HEARTBEAT_INVALID";
    case PSTOP_MESSAGE_TYPE_INVALID:  return "MESSAGE_TYPE_INVALID";
    case PSTOP_MISSED_HEARTBEATS:     return "MISSED_HEARTBEATS";
    case PSTOP_FATAL:                 return "FATAL";
    default:                          return "?";
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if      (!strcmp(argv[i], "-v")) force_verbose = 1;
        else if (argv[i][0] >= '0' && argv[i][0] <= '9') port_override = atoi(argv[i]);
    ...
    pstop_remote_data_t *pstop_clients =
        calloc(g_cfg.max_remotes, sizeof(pstop_remote_data_t));
    if (!pstop_clients) { fprintf(stderr, "OOM allocating %u remotes\n", g_cfg.max_remotes); return 1; }
    ...
    while (s_running) {
        ...
        result = transport_udp_read(&udp_transport, reqbytes,
                                     PSTOP_MESSAGE_SIZE, &client);
        ...
        if (result == PSTOP_MESSAGE_SIZE) {
            ...
            if (g_cfg.verbose) {
                fprintf(stderr,
                        ...
                        msg_name(req_msg.message), req_msg.counter,
                        req_msg.received_counter, req_msg.id.data,
            ...
            if (req_msg.checksum != req_msg.calculated_checksum) {
                ...
                fprintf(stderr,
                    "ANOMALY: bad checksum from wire (claimed 0x%08X) — dropped\n",
                    req_msg.id.data);
                ...
                if (req_msg.counter > last_rx_counter + 1U) {
                    fprintf(stderr,
                        ...
                        req_msg.id.data, last_rx_counter + 1U, req_msg.counter,
                    ...
                    fprintf(stderr,
                        ...
                        req_msg.id.data, last_rx_counter, req_msg.counter);
                }
                if (req_msg.stamp <= last_rx_stamp) {
                    fprintf(stderr,
                        "ANOMALY: stamp not monotonic from 0x%08X — last %llu, got %llu\n",
                        req_msg.id.data, (unsigned long long)last_rx_stamp,
                        (unsigned long long)req_msg.stamp);
                }
            ...
            pstop_error_t err = machine_process_message(
                &machine, &req_msg, &resp_msg);
            ...
            if (err == PSTOP_OK) {
                if (req_msg.message == PSTOP_MESSAGE_BOND ||
                    req_msg.message == PSTOP_MESSAGE_UNBOND) {
                    s_stop_episode_valid = 0;
                } else if (prev_restart != ROBOT_RESTART_STATE_STOP_RECEIVED &&
                           machine.robot_state.restart_state ==
                               ROBOT_RESTART_STATE_STOP_RECEIVED) {
                    /* Accepted STOP just opened an arming cycle. */
                    s_stop_episode_start = machine_now_ms();
                    s_stop_episode_valid = 1;
                }
                if (prev_robot == ROBOT_STATE_STOPPED &&
                    machine.robot_state.robot_state == ROBOT_STATE_OK) {
                    uint64_t held = s_stop_episode_valid
                        ? machine_now_ms() - s_stop_episode_start : 0U;
                    if (g_cfg.min_stop_ms > 0U &&
                        (!s_stop_episode_valid || held < g_cfg.min_stop_ms)) {
                        ...
                        fprintf(stderr,
                            ...
                            req_msg.id.data, (unsigned long long)held,
            ...
            if (err == PSTOP_OK) {
                ...
                transport_udp_write(&udp_transport, respbytes,
                                     PSTOP_MESSAGE_SIZE,
                                     (struct sockaddr_in *)&client);
                if (g_cfg.verbose)
                    fprintf(stderr, "  TX %-6s cnt=%u\n",
                ...
                if (c) {
                    ...
                    fprintf(stderr,
                        ...
                        msg_name(req_msg.message), err_name(err),
                        r2m, c->remote_data.last_sent_counter, req_msg.counter,
                        lag, c->remote_data.msg_counter, req_msg.received_counter);
                } else {
                    fprintf(stderr, "ANOMALY: %s rejected by pstop — %s (cnt=%u rcnt=%u; remote not bonded)\n",
                            msg_name(req_msg.message), err_name(err),
                            req_msg.counter, req_msg.received_counter);
                }
            }
            ...
            if (machine.robot_state.robot_state    != prev_robot   ||
                machine.robot_state.restart_state  != prev_restart ||
                ...
                fprintf(stderr,
                        ...
                        rstate_name(prev_robot), restart_name(prev_restart));
            }
        }
    }
