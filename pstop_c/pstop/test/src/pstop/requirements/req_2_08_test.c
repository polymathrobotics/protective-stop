...
// 2-08-1: Shall only accept OK (0), STOP (1), BOND (2), and UNBOND (3) messages
// Description: Send a message with a type that is not one of the above types.
...
req_2_08_test(void)
{
    ...
    req.message = 4U; // invalid supported message type
    req.counter = 10;
    req.stamp = 100;
...
void
main_req_2_08_test(void)
{
    UnitySetTestFile("req_2_08_test.c");

    RUN_TEST(req_2_08_test);
}
