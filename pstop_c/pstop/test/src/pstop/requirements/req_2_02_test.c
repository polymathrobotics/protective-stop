...
// 2-02-1: Shall be able to detect out of order messages
// Description: After a successful bonding, a message will be sent with
// a previously used counter value. The message shall be ignored. The
// last heartbeat timestamp shall not change for this device ID.

static
void
req_2_02_1_test(void)
{
    ...
    // now send message with counter that indicates a previous message
...
// 2-02-2: Shall be able to detect out of order messages
// Description: After a successful bonding, a message will be sent with
// a timestamp in the past. This message shall be ignored. The last
// heartbeat timestamp shall not change for this device ID.

static
void
req_2_02_2_test(void)
{
    pstop_machine_t machine;
    machine_init(&machine, &pstop_app, pstop_clients, MAX_CLIENTS);
    configure_app_defaults(&pstop_app, &MACHINE, 0U, 0U, 1000U);
...
void
main_req_2_02_test(void)
{
    UnitySetTestFile("req_2_02_test.c");

    RUN_TEST(req_2_02_1_test);
    RUN_TEST(req_2_02_2_test);
}
