...
// 3-05-1: Shall require valid messages (OK or STOP) from a bonded remote at
//         or faster than the machine specified heartbeat.
// Description: After a successful bonding and stop/ok sequence, send OK messages
// faster than the heartbeat timeout. Machine should stay in an OK state.
...
req_3_05_test(void)
{
    ...
    set_time(get_time() + 300U); // not a timeout
...
void
main_req_3_05_test(void)
{
    UnitySetTestFile("req_3_05_test.c");

    RUN_TEST(req_3_05_test);
}
