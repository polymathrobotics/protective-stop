...
{
  const auto snapshot = parse(R"({"relay_stop":false,"relay_fault_a":true,"relay_fault_b":true,"pstop_mismatch":7})");
...
{
  const auto snapshot = parse(R"({"relay_stop":true,"bonded_remotes":[)"
...
{
  const auto snapshot = parse(R"({"relay_stop":false,"bonded_remotes":[{"id":0,"state":0},{"id":5,"state":2}]})");
...
{
  const auto snapshot = parse(R"({"relay_stop":false,"bonded_remotes":[5,"x",{"id":9,"state":2}]})");
  ASSERT_EQ(snapshot.remotes.size(), 1u);
