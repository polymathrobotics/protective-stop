...
| **SR-R-03** | The two cores shall form the verdict by **provably non-convergent** expressions (core 0 arithmetic-image vs core 1 boolean), and the build shall guarantee this diversity survives optimization — verified by an object-code diff gate **or** compile-diverse translation units. | SG-1, **H-09**, **DU-3** | F-R-02 (Option B) | SIL 3 / PL e (systematic; common-cause leg) | **Fault-injection** (identical logic fault → encodings still diverge) + object-code diff | **Gap** (source-level diversity live at `:305/:307`; object-code non-identity **not** build-gated; HARA §7 item 5) |
...
| ID | Requirement (shall) | Derived from | Allocated to | Integrity | Verify | Status |
|---|---|---|---|---|---|---|
| **SR-M-01** | The node shall validate every timing parameter (initial and runtime set) against the safe floor envelope and **reject** any value that loosens the safe window; rejection shall not alter the running safe config. | SG-1/SG-3, H-04, FMEA M07-1/DU-4 | F-M-07 | SIL 3 / PL e | Test (reject loosening set-param) | **Satisfied** (floors declared as `generate_parameter_library` ranges in `protective_stop_machine_params.yaml`; the generated ParamListener rejects an out-of-floor override at declare time and a loosening set before it applies) |
...
**Top-priority Gaps (implement first — from OPEN_ITEMS §3b):** SR-H-03 (host
config floor, DU-4 — cheapest), SR-R-09 (GPIO re-verify, DU-1), SR-H-04
(frozen-clock, DU-2), SR-R-03 (Option-B object-code diff, DU-3), SR-R-08/SR-R-12
(comparator/encode common-mode, DU-5/7).
