...
| **R-03** | `PSTOP_SAFETY_DESIGN.md` §5.6 | Heartbeat timeout = `heartbeat_ms × (max_missed_heartbeats + 1)`; "today 1000 × 2 ≈ 1–2 s". | Library math is `heartbeat_ms × max_missed_heartbeats` (`machine.c:290-298`: OK while `diff ≤ hb`; STOP when `diff/hb ≥ max_missed`). Current defaults **400 × 3 ≈ 1.2 s** (SYSTEM_DEFINITION §2). | stale-value | **High** | Formula and values corrected directly in §5.6 with a code cite; original noted in an HTML comment. | yes |
...
| Doc | Verdict |
|---|---|
| `docs/safety/SYSTEM_DEFINITION.md` | Authoritative backbone — already reflects Option A+B / C-dropped, `hb×max_missed` (400×3≈1.2 s), SIL 3 / PL e. No change. |
...
| `docs/MACHINE_ROS2_NODE_DESIGN.md` | Uses `max_missed` correctly; defaults 400/3/500 (§4, §12). No stale heartbeat formula, no Option C, no SIL2. |
