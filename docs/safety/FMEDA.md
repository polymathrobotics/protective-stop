...
> series de-energize-to-safe stop (HFT=1) is unchanged, so **stop-on-demand is
> unaffected**; what degrades is stuck-on **diagnosis** (a single weld becomes
> latent-to-next-demand, T1 proof-test governed). The machine-channel SFF/PFH
> here therefore **overstate** the achieved integrity while feedback is
> descoped. Recompute is deferred to the same re-run, with safety-owner sign-off.
