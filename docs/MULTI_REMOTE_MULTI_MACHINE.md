...
3. **Optionally restrict admission — on the machine.** Both lists are empty by
   ...
   machine to *only listed remotes may bond*; `?deny=<id>` bans one regardless;
   `?unallow=`/`?undeny=` remove; `GET /api/admission` lists. **Allowlisting also
   PINS the remote on the machine (see §5) — it is what makes a cross-site remote
   reliably bond.** A refused remote is answered `UNBOND`, shows `REJECTED` on its
   machine-peer row, and does not retry until its **Rebond** button (or
   `POST /api/pstop_peers?slot=N&rebond=1`) is used.
4. **Verify** (see §4).

Provisioning at scale is the fleet's job: it stores each remote's machine slots and
role and each machine's admission lists and re-pushes them after any reflash
(NVS-preserving, but re-push to be safe). See the fleet-side handoff.
...
- **On the machine** — `GET /state.json` → `bonded_remotes[]`: each entry has
  ...
  stop-only — the remote's current announced role). `allowlist[]`/`denylist[]`
  show the admission lists.
...
- A **machine pins each admission-allowlisted remote** → the machine never trims
...
- An **allowlisted** remote will always bond to its machine, anywhere on the
  tailnet, because both sides pin each other. **Allowlist any remote that must
  reliably reach a machine across sites** (§3 step 3).
- An **unlisted** remote (open admission) is **best-effort** on a >128-node tailnet: if it has no
...
- **Keep the tailnet lean.** Retire stale/test nodes. Below 128 total nodes, nothing
  is trimmed and every remote bonds regardless of admission listing. `s_extra_pins`
...
1. **Is the tailnet > 128 nodes?** (`/admin/api/peers` length = 128 on both = at the
   ...
   (`POST /api/admission?allow=<id>`) so the machine pins it, or prune the tailnet
