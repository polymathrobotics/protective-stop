...
def encode(remote_id, message, stamp, received_stamp, counter, received_counter,
           corrupt=False):
    body = struct.pack("<BBQQIIIII", PSTOP_VERSION, message, stamp,
                       received_stamp, remote_id, MACHINE_ID, HEARTBEAT_MS,
...
def decode(data):
    (_ver, message, stamp, _rs, _snd, _rcv, _hb, counter,
     received_counter) = struct.unpack("<BBQQIIIII", data[:38])
...
class SoftRemote:
    ...
    def send(self, message, corrupt=False, bad_type=None):
        ...
        pkt = encode(self.id, msg, self._now(), self.last_rx_stamp,
                     self.counter, self.last_rx_counter, corrupt=corrupt)
        try:
            self.sock.send(pkt)
    ...
    def run(self, duration_s, assign):
        ...
        while time.monotonic() < end:
            for r, m in assign.items():
                r.send(m)
            time.sleep(0.1)
        return {r.id: (r.last_reply["message"] if r.last_reply else None)
                for r in assign}
...
class Machine:
    ...
    def __init__(self, toml_text=None, verbose=False):
        ...
        self.proc = subprocess.Popen(args, stdout=subprocess.DEVNULL,
                                     stderr=self.log, cwd=os.path.join(HERE, ".."))
        # wait for "listening"
...
def group_a_single(verbose):
    ...
    with Machine(verbose=verbose) as m:
        ...
        check(r[a.id] == MSG_STOP, "A1 OK-stream before arming stays STOP (I3)",
              NAMES.get(r[a.id]))
        ...
        s.run(0.2, {a: MSG_STOP})
        r = s.run(1.2, {a: MSG_OK})
        check(r[a.id] == MSG_STOP, "A4 blip while armed stops + release vetoed",
              NAMES.get(r[a.id]))
        r = arm(s, a, [])
...
def group_b_two_remote(verbose):
    ...
    with Machine(verbose=verbose) as m:
        ...
        check(r[a.id] == MSG_STOP and r[b.id] == MSG_STOP,
              "B1 both bonded, no arming -> both STOP (I3)",
              f"A={NAMES.get(r[a.id])} B={NAMES.get(r[b.id])}")
        r = arm(s, a, [b])
        check(r[a.id] == MSG_OK and r[b.id] == MSG_OK,
...
def group_c_heartbeat(verbose):
    ...
    with Machine(verbose=verbose) as m:
        ...
        check(r[a.id] == MSG_STOP,
              "C1 remote B silent > timeout -> robot STOP (I2)", NAMES.get(r[a.id]))
...
def group_d_unbond(verbose):
    ...
    with Machine(verbose=verbose) as m:
        ...
        check(r[b.id] == MSG_STOP,
              "D1 arming owner unbonds -> robot STOP/NEED_STOP (I7)",
              NAMES.get(r[b.id]))
        # After owner left, B may now take ownership and arm.
        r = arm(s, b, [])
        check(r[b.id] == MSG_OK,
              "D2 ownership released on unbond -> B can now arm", NAMES.get(r[b.id]))
        # B unbonds (last remote) -> stays stopped, no crash.
        b.unbond()
        r = s.run(0.5, {b: MSG_STOP})
        check(True, "D3 last remote unbond handled (no crash)",
              "machine alive" if m.proc.poll() is None else "DIED")
        a.close(); b.close()
...
def group_e_capacity_allow(verbose):
    ...
    with Machine(verbose=verbose) as m:
        ...
        check(not got, "E1b 4th remote rejected at capacity (I6 OUT_OF_OPERATOR_SPACE)",
              "bonded!" if got else "rejected")
        # existing remotes still serviced
        s = Session(rs)
        r = s.run(0.6, {rs[0]: MSG_OK, rs[1]: MSG_OK, rs[2]: MSG_OK})
        check(all(v is not None for v in r.values()),
              "E1c existing 3 remotes still serviced after overflow")
        for r_ in rs:
            r_.close()
        fourth.close()
    ...
    with Machine(toml_text=toml, verbose=verbose) as m:
        ...
        check(not denied.bond(timeout_s=3),
              "E2b unlisted operator rejected (OPERATOR_NOT_ALLOWED)")
        allowed.close(); denied.close()
    ...
    with Machine(toml_text=toml, verbose=verbose) as m:
        ...
        check(r[a.id] == MSG_STOP,
              "E3a stop_only remote cannot arm the robot",
              f"A={NAMES.get(r[a.id])}")
        # but it CAN still stop: A arms, then stop_only stops -> STOP
        arm(s, a, [so])
        r = s.run(1.0, {a: MSG_OK, so: MSG_STOP})
        check(r[a.id] == MSG_STOP,
              "E3b stop_only remote can still STOP the robot (I1)",
              f"A={NAMES.get(r[a.id])}")
        a.close(); so.close()
    ...
    with Machine(verbose=verbose) as m:
        ...
        while time.monotonic() < end:
            ...
            if a.last_reply and a.last_reply["message"] == MSG_STOP:
                saw_stop = True
            time.sleep(0.1)
        check(saw_stop, "F1 sustained UNKNOWN from B -> robot STOP via timeout (I2)",
        ...
        while time.monotonic() < end:
            ...
            if a.last_reply and a.last_reply["message"] == MSG_STOP:
                saw_stop = True
            time.sleep(0.1)
        check(saw_stop, "F2 sustained bad-CRC from B -> robot STOP (I2, no spurious OK)",
              "STOP seen" if saw_stop else "never stopped")
        check("checksum" in m.stderr().lower(),
              "F2b bad checksum rejected/logged by machine")
        ...
        for _ in range(8):
            ...
            a.send(MSG_OK)
            time.sleep(0.1)
        check(m.proc.poll() is None and (not a.last_reply or a.last_reply["message"] == MSG_STOP),
...
def group_g_chaos(verbose):
    ...
    with Machine(verbose=verbose) as m:
        ...
        proxy = subprocess.Popen(
            [sys.executable, proxy_py, "--listen", str(listen),
             "--fwd", str(m.port), "--fwd-host", "127.0.0.1", "--ctrl", str(ctrl)],
        ...
        try:
            ...
            r = arm(s, a, [b])
            check(r[a.id] == MSG_OK and r[b.id] == MSG_OK,
                  "G1 arm through proxy -> OK", f"A={NAMES.get(r[a.id])}")
            # Heavy loss: heartbeats can't get through -> machine must STOP.
            chaos("loss=0.9")
            saw_stop = False
            end = time.monotonic() + 3.5
            while time.monotonic() < end:
                a.send(MSG_OK); b.send(MSG_OK)
                if a.last_reply and a.last_reply["message"] == MSG_STOP:
                    saw_stop = True
                time.sleep(0.1)
            check(saw_stop or "STOP" in m.stderr(),
            ...
            r = arm(s, a, [b])
            check(r[a.id] == MSG_OK, "G3 recovery after loss clears -> re-arm OK",
                  f"A={NAMES.get(r[a.id])}")
            ...
            while time.monotonic() < end:
                a.send(MSG_OK); b.send(MSG_STOP)  # B holds STOP the whole time
...
def main():
    ...
    groups = [group_a_single, group_b_two_remote, group_c_heartbeat,
              group_d_unbond, group_e_capacity_allow, group_f_malformed,
              group_g_chaos]
    for g in groups:
        try:
            g(args.verbose)
