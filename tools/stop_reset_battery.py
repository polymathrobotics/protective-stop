...
def spawn_runner(port):
    ...
    cfg.close()
    p = subprocess.Popen([RUNNER, cfg.name, str(port)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    if p.poll() is not None:
        sys.exit(
            f'machine_app_runner exited immediately — is port {port} already in use? '
...
class Remote:
    ...
    def _decode(self, data):
        (ver, message, stamp, rstamp, sender, receiver, hb, counter, rcounter) = struct.unpack('<BBQQIIIII', data[:38])
        (chk,) = struct.unpack('<H', data[38:40])
        return dict(message=message, stamp=stamp, counter=counter, crc_ok=(chk == crc16(data[:38])))
...
def main():
    print(
        f'pstop stop/reset battery vs machine on 127.0.0.1:{PORT}\n'
    ...
    stale = r.xfer(MSG_OK)
    check(
        stale is None or stale['message'] in (MSG_STOP, MSG_UNBOND),
