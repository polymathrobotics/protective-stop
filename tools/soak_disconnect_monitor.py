...
def run(args):
    ...
    sample_hdr = ["ts", "elapsed_s", "remote", "slot_state", "sent", "replies",
                  "send_fail", "rebonds", "rtt_ms", "wg_direct", "ml_state",
                  "ml_reconnects", "eth_link", "eth_recoveries"]
    telem_hdr = ["ts", "unit", "ml_reconnects", "ml_state", "slot_rtt_ms",
                 "slot_wg_direct", "rebonds", "replies", "send_fail",
                 "derp_home_region", "pp_has_direct"]
    ...
    log_event(f"[{now()}] === disconnect soak START "
    ...
    while time.time() - t0 < args.duration:
        ...
        for rname, url in remotes.items():
            ...
            if "__err__" in st:
                ...
                log_event(f"[{now()}] EVENT #{event_counts[rname]} {rname}: "
            ...
            if lp:
                ...
                if (cur["slot_state"] == BOND_STATE_BONDED
                        and cur["sent"] > lp["sent"]
                        and stall > args.reply_stall_sec):
                    trig.append(f"reply STALL {int(stall)}s (sent advancing, replies frozen)")

                if trig:
                    event_counts[rname] += 1
                    last_event_wall = time.time()
                    log_event(f"\n[{now()}] ===== EVENT #{event_counts[rname]} "
                              f"{rname} (elapsed={elapsed}s) =====")
                    log_event(f"  triggers: {'; '.join(trig)}")
                    log_event(f"  {rname} slot: state={cur['slot_state']} "
                    ...
                    log_event(f"  {rname} net:  ml_state={st.get('ml_state')} "
            ...
            with open(samples_path, "a") as f:
                f.write(",".join(str(x) for x in [
                    now(), elapsed, rname, cur["slot_state"], cur["sent"],
                    cur["replies"], cur["send_fail"], cur["rebonds"],
                    cur["rtt_ms"], cur["wg_direct"], cur["ml_state"],
                    cur["ml_reconnects"], cur["eth_link"], cur["eth_recoveries"],
                ]) + "\n")
        ...
        with open(status_path, "w") as f:
            f.write(f"[{now()}] elapsed={elapsed}s/{args.duration}s  "
                    f"events: {dict(event_counts)}\n")
            clean_s = int(time.time() - last_event_wall)
            f.write(f"  clean streak: {clean_s}s"
                    + (f" / target {args.clean_target}s" if args.clean_target else "")
                    + "\n")
            for rname, lp in last.items():
                f.write(f"  {rname}: state={lp['slot_state']} replies={lp['replies']} "
        ...
        if args.clean_target and (time.time() - last_event_wall) >= args.clean_target:
            log_event(f"[{now()}] === CLEAN TARGET MET: {args.clean_target}s with "
    ...
    log_event(f"[{now()}] === soak END: events={dict(event_counts)} "
              f"(total={total}) ===")
...
def build_parser():
    ...
    p.add_argument("--remote", action="append", metavar="NAME=URL", required=True,
                   help="A pstop remote to watch, e.g. PSTOP54=http://192.168.107.70. "
                        "Repeatable.")
    p.add_argument("--machine", action="append", metavar="NAME=URL", default=[],
                   help="A pstop machine node, e.g. machn=http://100.84.155.111. "
                        "Repeatable. Used for the bonded_remotes context in events.")
    p.add_argument("--machine-id", type=lambda s: int(s, 0), default=None,
    ...
    p.add_argument("--admin-user", default=None,
                   help="Admin user for /admin/api/monitor (default env ML_ADMIN_USER "
                        "or 'admin').")
    ...
    p.add_argument("--machine-admin-pass", default=None,
                   help="Admin password for machine nodes if different from remotes "
                        "(default env ML_MACHINE_ADMIN_PASSWORD).")
    p.add_argument("--poll-sec", dest="poll_sec", type=float, default=5.0,
                   help="Seconds between full poll cycles (default 5).")
    p.add_argument("--telem-sec", dest="telem_sec", type=float, default=30.0,
                   help="Seconds between telemetry.csv rows (default 30).")
    p.add_argument("--reply-stall-sec", dest="reply_stall_sec", type=float,
                   default=DEFAULT_REPLY_STALL_S,
                   help="A bonded slot with sent advancing but replies frozen this "
                        "long is a stall event (default 8).")
    p.add_argument("--duration", type=float, default=43200,
                   help="Total run time in seconds (default 43200 = 12h).")
    p.add_argument("--out", default="./soak_run",
                   help="Output directory for events.log / samples.csv / telemetry.csv "
                        "/ status.txt (default ./soak_run).")
    p.add_argument("--serial", default=None, metavar="DEV",
                   help="Optional USB console device (e.g. /dev/ttyACM1) to capture "
                        "and tail into each event record.")
    p.add_argument("--on-disconnect", default=None, metavar="CMD",
                   help="Optional shell command run once per detected event "
                        "(generic hook; e.g. to reset a HIL fixture).")
    p.add_argument("--clean-target", dest="clean_target", type=float, default=None,
                   metavar="SECONDS",
