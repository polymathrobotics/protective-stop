...
def run(args):
    ...
    machine_admin_pass = args.machine_admin_pass or os.environ.get(
        "ML_MACHINE_ADMIN_PASSWORD"
    )
    ...
    while time.time() - t0 < args.duration:
        ...
        remote_states = {
            n: http_get_json(u + "/state.json") for n, u in remotes.items()
        }
        machine_states = {
            n: http_get_json(u + "/state.json") for n, u in machines.items()
        }
        ...
        for rname, url in remotes.items():
            ...
            if lp:
                trig = []
                if (
                    lp["slot_state"] == BOND_STATE_BONDED
                    and cur["slot_state"] != BOND_STATE_BONDED
                ):
                    trig.append(
                        f"peer bond DROPPED {lp['slot_state']}->{cur['slot_state']}"
                    )
                if cur["rebonds"] > lp["rebonds"]:
                    trig.append(f"rebonds {lp['rebonds']}->{cur['rebonds']}")
                if cur["ml_reconnects"] > lp["ml_reconnects"]:
                    trig.append(
                        f"ml_reconnects {lp['ml_reconnects']}->{cur['ml_reconnects']}"
                    )
                ...
                if (
                    cur["slot_state"] == BOND_STATE_BONDED
                    and cur["sent"] > lp["sent"]
                    and stall > args.reply_stall_sec
                ):
                    trig.append(
                        f"reply STALL {int(stall)}s (sent advancing, replies frozen)"
                    )
        ...
        if time.time() - t0 >= next_telem:
            ...
            for uname, url in all_units:
                ...
                if pw:
                    mon = http_get_json(
                        url + "/admin/api/monitor", admin_user, pw, timeout=4
                    )
                    ...
                    f.write(
                        ",".join(
                            ...
                            for x in [
                                ...
                                mon.get("derp_home_region", "")
                                if isinstance(mon, dict)
                                else "",
                                mon.get("pp_has_direct", "")
                                if isinstance(mon, dict)
                                else "",
                            ]
                        )
        ...
        with open(status_path, "w") as f:
            f.write(
                f"[{now()}] elapsed={elapsed}s/{args.duration}s  events: {dict(event_counts)}\n"
            )
            clean_s = int(time.time() - last_event_wall)
            f.write(
                f"  clean streak: {clean_s}s"
    ...
    log_event(
        f"[{now()}] === soak END: events={dict(event_counts)} (total={total}) ==="
    )
