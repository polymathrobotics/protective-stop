...
class Stats:
    def __init__(self):
        self.lock = threading.Lock()
        self.c = {k: 0 for k in
                  ("fwd", "ret", "drop_up", "drop_down", "dup_up", "dup_down",
...
def main():
    ...
    def ctrl_loop():
        while True:
            ...
            print(f"[ctrl] {line.decode(errors='replace').strip()} -> "
                  f"base={imp.base} up={imp.up} down={imp.down}", flush=True)

    for fn in (front_loop, back_loop, ctrl_loop):
        threading.Thread(target=fn, daemon=True).start()

    print(f"chaos proxy: chip->:{args.listen} => {args.fwd_host}:{args.fwd}, "
          f"ctrl 127.0.0.1:{args.ctrl}", flush=True)
    while True:
        time.sleep(10)
