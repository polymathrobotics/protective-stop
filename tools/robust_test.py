...
"""
import urllib.request, json, time, subprocess, base64, sys
...
def page_latency():
    try:
        o = subprocess.run(["curl", "-m", "15", "-s", "-o", "/dev/null",
                            "-w", "%{time_connect} %{time_total}", f"http://{CHIP}/"],
...
def fmt(v):
    ...
    return f"n={len(s)} min={s[0]:.0f} med={p(.5):.0f} p95={p(.95):.0f} max={s[-1]:.0f}"
...
for r in range(REBOOTS):
    ...
    if ts_t:
        boot_ts.append(ts_t)
    log(f"   reachable={reach_t and round(reach_t,1)}s tailscale={ts_t and round(ts_t,1)}s "
    ...
    while time.time() - t0 < SPACING:
        ...
        if d:
            ...
            if last_up is not None and up < last_up - 5 and up < 30:
                ...
                log(f"   >>> SPONTANEOUS REBOOT uptime {last_up}->{up}s reset_reason={rr2}"
                    + ("  *** TWDT ***" if rr2 == 4 else ""))
                if rr2 == 4:
                    twdt += 1
            last_up = up
