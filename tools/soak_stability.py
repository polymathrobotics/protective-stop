...
"""Read-only, one-phase DUT/peer armed-soak collector (stdlib only).

See docs/SOAK_COLLECTOR.md for the peer contract and invocation. Credentials use
ML_ADMIN_USER/ML_ADMIN_PASSWORD. Only GETs are issued; an optional failure hook
receives an event path without a shell. Exit 0 means the consecutive target was
verified, 2 means incomplete/error, and 130/143 mean interrupted.

Restarts preserve evidence but start a new window. Observation gaps and faults
reset credit; a fresh HTTP response never extends an expired reply certificate.
"""
