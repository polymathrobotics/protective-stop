...
def udev_prop(port, key):
    try:
        out = subprocess.run(
            ['udevadm', 'info', '-q', 'property', '-n', port], capture_output=True, text=True, timeout=5
        ).stdout
...
def running_ports():
    ...
    for base in ('/dev/ttyACM', '/dev/ttyUSB'):
        for n in range(8):
            ...
            if (
                os.path.exists(p)
                and udev_prop(p, 'ID_VENDOR_ID') == ESP_VID
...
def wait_for_blank(known):
    ...
    while True:
        ...
        if fresh:
            hinted.update(fresh)
            line(
                f'  {C.Y}a running pstop is on {", ".join(fresh)} — to reflash it, '
                f'hold BOOT and tap RESET (stored config is preserved).{C.X}'
            )
        sys.stdout.write(f'\r  {C.DIM}waiting for an ESP32 in download mode {spin[i % 4]}{C.X}   ')
        sys.stdout.flush()
        i += 1
        time.sleep(0.4)
...
def read_mac(cmd, port, v5=False, tries=READ_MAC_TRIES):
    ...
    for i in range(tries):
        try:
            out = subprocess.run(
                cmd + ['--chip', CHIP, '--connect-attempts', CONNECT_ATTEMPTS, '-p', port, sub],
                capture_output=True,
                text=True,
                timeout=40,
            )
            ...
            last = str(e)[:80]
        if i + 1 < tries:
            line(f"  {C.DIM}chip didn't answer on {port} ({last}); retry {i + 2}/{tries}…{C.X}")
...
def flash(cmd, port, v5, erase, on_progress):
    ...
    if erase:
        ...
        r = subprocess.run(
            cmd + ['--chip', CHIP, '--connect-attempts', CONNECT_ATTEMPTS, '-p', port, '-b', BAUD, EF],
            capture_output=True,
            text=True,
        )
        if r.returncode != 0:
    ...
    for attempt in range(1, FLASH_TRIES + 1):
        ...
        if attempt < FLASH_TRIES:
            line(f'  {C.DIM}flash attempt {attempt}/{FLASH_TRIES} failed ({last_err[-80:]}); retrying…{C.X}')
...
def ts_ip(host):
    try:
        out = subprocess.run(['tailscale', 'ip', '-4', host], capture_output=True, text=True, timeout=8)
...
def reset_peers_fleet_only(ip):
    ...
    for s in range(PEER_SLOTS):
        try:
            req = urllib.request.Request(f'http://{ip}/api/pstop_peers?slot={s}&clear=1', method='POST')
...
def confirm_hardware(ip):
    ...
    line(
        f'    1. LED ring: on boot it plays a dim {C.B}purple comet{C.X}, then '
        f'settles to {C.BOLD}solid white{C.X} (idle — no peer). Seeing that = ring OK.'
    )
    line(
        f'    2. Button: open {C.B}http://{ip}/{C.X} and press/release the STOP '
    ...
    while True:
        ans = (
            input(f'  {C.BOLD}Ring (purple comet → white) AND button panel both work? [y/N/r=retry link] {C.X}')
            .strip()
            .lower()
        )
        if ans == 'r':
...
def flash_one(cmd, v5, port, erase, ip_timeout, app_ver='?', keep_peers=False):
    ...
    if not ip:
        line(
            f'  {C.R}✗ no tailnet IP for {host} within {ip_timeout}s (check DHCP / Tailscale key / Ethernet link).{C.X}'
        )
    ...
    st = unit_state(ip)
    line(
        f'  {C.G}✓ online at {C.BOLD}{ip}{C.X}{C.G} '
    ...
    if app_ver != '?' and run_fw and not run_fw.startswith(app_ver):
        line(f'  {C.Y}! running fw_ver {run_fw} != flashed {app_ver} — boot slot / stale image?{C.X}')
...
def selftest(cmd, v5):
    ...
    hosts = ts_status_hosts()
    print(f'  tailscale sees {len(hosts)} pstop nodes; online: {[h for h, up in hosts.items() if up] or "none"}')
    for h in list(hosts)[:1]:
        print(f'  ts_ip({h}) = {ts_ip(h)}')
    if _prov is None:
        print(f'  build source: {C.Y}no plugin (tools/app_source.py) — staged image only{C.X}')
    else:
        ok, msg = _prov.status()
...
def main():
    ...
    ap.add_argument(
        '--from-image', action='store_true', help='flash the locally-staged app; ignore the build-source plugin'
    )
    ap.add_argument('--fw-sha', metavar='PREFIX', help='pin a specific build by sha256 prefix (build-source plugin)')
    ap.add_argument('--project', default='pstop_remote', help='build-source project name (default pstop_remote)')
    ap.add_argument('--no-refresh', action='store_true', help="fetch latest once at startup, don't re-check per unit")
    ap.add_argument(
    ...
    if use_plugin:
        ...
        if not img:
            sys.exit(
                f'{C.R}ERROR: {_prov.LABEL} could not supply an app image '
        ...
        print(
            f'  app: {C.G}{_prov.LABEL} {app_ver}{C.X} ({app_sha[:12]}, '
            f'{"starred" if img.get("starred") else "newest"})' + (' [pinned]' if args.fw_sha else '')
        )
    else:
        if not os.path.isfile(APP_BIN):
            sys.exit(
                f'{C.R}ERROR: no app image — {APP_BIN} is not staged'
                + (' and no build-source plugin is present' if _prov is None else '')
                + f'.{C.X}'
            )
        why = '--from-image' if args.from_image else 'no build-source plugin'
        print(f'  app: {C.Y}local staged image{C.X} ({why})')
    cfg_note = (
        f'{C.R}--erase WILL wipe stored config (NVS){C.X}' if args.erase else 'reflash preserves stored config (NVS)'
    )
    print(
        f'  Plug in an ESP32-S3 in download mode — blank, or an already-flashed '
    ...
    try:
        while True:
            ...
            print(
                f'{C.DIM}  session: {C.G}{n_ok} PASS{C.DIM}, '
        ...
        print()
    print(f'\n{C.BOLD}=== session summary ==={C.X}  {C.G}{n_ok} passed{C.X}, {C.R}{n_fail} failed{C.X}')
    for f in fails:
        print(
            f'  {C.R}✗{C.X} {f.get("node", f.get("port"))} — failed at '
