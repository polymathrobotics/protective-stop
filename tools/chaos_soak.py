...
def icmp(host):
    try:
        o = subprocess.run(['ping', '-c', '1', '-W', '1', host], capture_output=True, text=True, timeout=3)
        if o.returncode == 0:
...
def ensure_field(field, want_on):
    def f():
        ...
        path = {
            'eth_en': '/api/iface/eth',
            'wifi_en': '/api/iface/wifi',
            'usbncm_en': '/api/iface/usb',
            'wg_paused': '/api/wg',
        }[field]
        post(path, h)
...
def fmt(vs):
    ...
    mean = sum(s) / n
    return f'n={n} min={s[0]:.1f} mean={mean:.1f} p50={q(0.5):.1f} p95={q(0.95):.1f} p99={q(0.99):.1f} max={s[-1]:.1f}'
...
CHAOS = ['eth_short', 'wg_disrupt', 'eth_long', 'usb_toggle', 'reboot', 'wifi_manual', 'eth_flap', 'reboot']
    ...
    try:
        ...
        if d:
            ...
            if last_up is not None and up < last_up - 5 and up < 40:
                ...
                log(
                    f'  >>> REBOOT detected uptime {last_up}->{up}s '
                    f'{"[UNEXPECTED — crash]" if unexpected else "[intentional]"}'
                )
            last_up = up
            SAMP.write(
                f'{int(c0 - start)},1,{r if r is not None else ""},{hrtt:.1f},'
        ...
        if c0 >= next_chaos:
            ...
            if act == 'reboot':
                ...
                log(f'CHAOS #{ci}: Ethernet DROP 12s')
                M['events'] += 1
            elif act == 'eth_long':
                post('/api/iface/eth', h) if d and d.get('eth_en') else None
                defer(40, ensure_field('eth_en', True), 'eth on')
                log(f'CHAOS #{ci}: Ethernet DROP 40s (force WiFi failover)')
                M['events'] += 1
            elif act == 'eth_flap':
                post('/api/iface/eth', h) if d and d.get('eth_en') else None
        ...
        if M['samples'] % 30 == 0:
            ...
            log(
                f'... t={int(c0 - start)}s reach={sr:.1f}% icmp[{fmt(M["icmp"])}] '
...
for fn, desc in (
    (ensure_field('eth_en', True), 'eth on'),
...
log(f'reachability       : {sr:.2f}%  ({M["reach"]}/{M["samples"]})  [downtime incl. intentional reboots/outages]')
log(f'longest outage     : {M["max_unreach"] * SAMPLE:.0f}s')
log(f'ICMP ping RTT (ms) : {fmt(M["icmp"])}')
log(f'HTTP RTT (ms)      : {fmt(M["http"])}')
log(
    f'pstop path         : sent+{psent} replies+{prepl}  reply-rate {prate:.1f}% '
    f'(note: dual-core redundant send, ~50% nominal)'
)
rebonds_d = M['rebonds_last'] - (M['rebonds0'] or 0)
log(f'pstop rebonds      : +{rebonds_d}  (link re-syncs; chaos-driven is expected, a flat baseline is the goal)')
log(f'lockstep mismatch  : max {M["mm_max"]}')
log(
    f'reboots            : observed {M["reboots_obs"]}  intentional {M["reboots_int"]}  '
...
log('raw samples        : /tmp/chaos_samples.csv')
log('=' * 64)
SAMP.close()
