...
def test_single_open_channel_stops_via_liveness(wired, machine, rig, chip, cfg, channel, idx):
    ...
    assert not machine.find(RX_STOP_RE, since=since), (
        f'chip sent a STOP with only channel {channel.upper()} open — comparator should have withheld'
    )
    ...
    time.sleep(2.5)
    assert machine.robot_status(since) != 'OK', 'machine re-armed on single-channel heal without an arming gesture'
