...
def test_fresh_machine_needs_gesture(wired, machine, rig, cfg):
    ...
    time.sleep(1.5)
    assert machine.robot_status() != 'OK', 'machine armed from a fresh bond with no press/release gesture'
...
def test_press_stops_release_arms(wired, machine, rig, cfg):
    ...
    assert held_ms >= min_stop * 0.9, f'armed {held_ms:.0f} ms after STOP — violates min_stop_ms={min_stop}'
