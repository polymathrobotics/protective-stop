...
def discover_chip_ip(iface: str = "esp-pstop0", timeout: float = 15.0) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        if r.returncode != 0:
            ...
            candidates = [
                ln.split()[0] for ln in r.stdout.splitlines() if "lladdr" in ln
            ]
            if len(candidates) == 1:
                return candidates[0]
...
class Chip:
    ...
    def wait_state(
        self, pred, timeout: float, poll: float = 0.25, what: str = "condition"
    ):
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(poll)
        raise TimeoutError(
            f"chip: {what} not met within {timeout}s (last error: {last_exc})"
        )
    ...
    def set_peer_slot(self, slot: int, ip: str, port: int, machine_id: int) -> None:
        ...
        resp = json.loads(
            self._post(
                f"/api/pstop_peers?slot={slot}&ip={ip}&port={port}&id=0x{machine_id:08X}"
            )
        )
...
class MachineRunner:
    ...
    def __init__(
        self, binary: Path, workdir: Path, port: int, device_id: int, timing: dict
    ):
